...
/* trace point for hmp thresholds */
TRACE_EVENT(set_hmp_thresholds,

	TP_PROTO(struct task_struct *p, unsigned int down, unsigned int up),
	...
	TP_STRUCT__entry(
		__array(	char,	comm,	TASK_COMM_LEN	)
		__field(	pid_t,	pid			)
		__field(	unsigned int,	down		)
		__field(	unsigned int,	up		)
	),
	...
	TP_printk("comm=%s pid=%d down_threshold=%u up_threshold=%u",
		  __entry->comm, __entry->pid,
		  __entry->down, __entry->up)
);

/* Tracepoint for hmp boost */
TRACE_EVENT(hmp_boostpulse,

	TP_PROTO(struct task_struct *p, int value),
	...
	TP_printk("comm=%s pid=%d duration=%dus",
		  __entry->comm, __entry->pid, __entry->value)
);

/* Tracepoint for hmp boost */
TRACE_EVENT(hmp_boost,

	TP_PROTO(struct task_struct *p, int enable, int value),

	TP_ARGS(p, enable, value),
	...
	TP_printk("comm=%s pid=%d enable=%d value=%d",
		  __entry->comm, __entry->pid,
		  __entry->enable, __entry->value)
);
#endif
