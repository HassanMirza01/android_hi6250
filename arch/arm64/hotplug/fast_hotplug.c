...
static int max_cpus_online = CONFIG_NR_CPUS;
module_param(max_cpus_online, int, 0644);
static int min_cpus_online = 1;
module_param(min_cpus_online, int, 0644);
...
static void plug_in(int online_cpu_count){
	...
	if(online_cpu_count >= max_cpus_online)
		return;
...
static void plug_out(int online_cpu_count){
	...
	if(online_cpu_count <= min_cpus_online)
		return;
