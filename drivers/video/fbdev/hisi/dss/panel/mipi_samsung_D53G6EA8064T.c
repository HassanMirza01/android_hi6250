...
static unsigned int g_vr_mode_local = VR_MODE_DISABLE;
...
static struct dsi_cmd_desc vr_cycle_aid1_aor70_cmd[] = {
...
static struct dsi_cmd_desc vr_cycle_aid4_aor_orig_cmd[] = {
	{DTYPE_DCS_LWRITE, 0, 10, WAIT_TYPE_US,
		sizeof(test_key_enable), test_key_enable},
	{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
		sizeof(global_para1), global_para1},
...
static int mipi_samsung_D53G6EA8064T_panel_on(struct platform_device *pdev)
{
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		if(runmode_is_factory()) {
			...
			/* vr mode param setting */
			if (VR_MODE_ENABLE == g_vr_mode_local){
				HISI_FB_INFO("enable vr mode\n");
				mipi_dsi_cmds_tx(vr_cycle_aid1_aor70_cmd,
						ARRAY_SIZE(vr_cycle_aid1_aor70_cmd), mipi_dsi0_base);
			}else if (VR_MODE_DISABLE == g_vr_mode_local){
				HISI_FB_INFO("disable vr mode, no need to set, use default settings\n");
			}else{
				HISI_FB_INFO("unknown vr mode operate\n");
			}
		}
	...
	const char *buf, size_t count)
{
	...
	ret = strict_strtoul(buf, 0, &val);
	if (ret){
		HISI_FB_ERR("string to l int error, buf[%s], ret:%d\n", buf, ret);
		return count;
	}
	flag = (int)val;
	...
	if (SAMSUNG_VR_CYCLE_AID1_AOR70 == flag){
		...
		mipi_dsi_cmds_tx(vr_cycle_aid1_aor70_cmd,
				ARRAY_SIZE(vr_cycle_aid1_aor70_cmd), mipi_dsi0_base);
		g_vr_mode_local = VR_MODE_ENABLE;
	}else if (SAMSUNG_VR_CYCLE_AID4_AOR80 == flag){
		HISI_FB_INFO("VR CYCLE SET normal mode\n");
		mipi_dsi_cmds_tx(vr_cycle_aid4_aor_orig_cmd,
				ARRAY_SIZE(vr_cycle_aid4_aor_orig_cmd), mipi_dsi0_base);
		g_vr_mode_local = VR_MODE_DISABLE;
	}else{
...
static int mipi_samsung_D53G6EA8064T_probe(struct platform_device *pdev)
{
	...
	g_lcd_support_mode = COMFORM_MODE | IC_COLOR_ENHANCE_MODE | VR_MODE;
	pdev->id = 1;
	// init lcd panel info
