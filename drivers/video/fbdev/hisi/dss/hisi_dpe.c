...
int dpe_regulator_enable(struct hisi_fb_data_type *hisifd)
{
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970) {
		outp32(hisifd->media_crg_base + MEDIA_PERRSTDIS1, 0x000000F0);  //TODO
	}
#endif
...
static int dpe_on(struct platform_device *pdev)
{
	...
	if ((g_dss_version_tag == FB_ACCEL_KIRIN970_ES) || (g_dss_version_tag == FB_ACCEL_KIRIN970)) {
		dpe_regulator_enable(hisifd);
	#if defined (CONFIG_HISI_FB_970)
		dpe_set_common_clk_rate(pdev);
	#endif
		dpe_common_clk_enable(hisifd);
		dpe_inner_clk_enable(hisifd);
	}
...
static ssize_t dpe_lcd_reg_read_store(struct platform_device *pdev,
	const char *buf, size_t count)
{
	ssize_t ret = -1;
	struct hisi_fb_data_type *hisifd = NULL;
