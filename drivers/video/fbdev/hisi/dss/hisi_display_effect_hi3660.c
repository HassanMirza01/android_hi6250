...
static char effect_ce_gallery[2] = {'2','\0'};
...
{
	if (hisifd->hiace_info.gradual_frames > 0) {
		return;
	}
...
int hisifb_ce_service_set_lut(struct fb_info *info, void __user *argp)
{
	...
	if (hiace_set_interface.result == 0) {
		...
		EFFECT_DEBUG_LOG(DEBUG_EFFECT_FRAME, "[effect] bl:%d\n", hiace_set_interface.backlight);
		mutex_lock(&(bl_ctrl->ctrl_lock));
		bl_ctrl->ctrl_bl_delta = hiace_set_interface.backlight;
...
ssize_t hisifb_display_effect_ce_ctrl_store(struct fb_info *info, const char *buf, size_t count)
{
	...
	if (mode != ce_ctrl->ctrl_ce_mode) {
		if (mode == CE_MODE_DISABLE && metadata_ctrl->metadata[GALLERY_CTRL_BIT] != GALLERY_OFF) {
			mode = CE_MODE_IMAGE;
		}
		...
		down(&hisifd->blank_sem);
		hisifb_activate_vsync(hisifd);
		if (mode == CE_MODE_VIDEO) {
			if (hisifd->panel_power_on && pdata->lcd_ce_mode_store) {
				if(ERR_RET == pdata->lcd_ce_mode_store(hisifd->pdev, LCD_CE_MODE_VIDEO, sizeof(LCD_CE_MODE_VIDEO))) {
					HISI_FB_ERR("[effect] lcd_ce_mode_store failed\n");
				}
			}
		} else if (mode == CE_MODE_DISABLE) {
			if (hisifd->panel_power_on && pdata->lcd_ce_mode_store) {
				if(ERR_RET == pdata->lcd_ce_mode_store(hisifd->pdev, LCD_CE_MODE_DEFAULT, sizeof(LCD_CE_MODE_DEFAULT))) {
					HISI_FB_ERR("[effect] lcd_ce_mode_store failed\n");
				}
			}
		} else if (mode == CE_MODE_IMAGE) {
			if (hisifd->panel_power_on && pdata->lcd_ce_mode_store) {
				if(ERR_RET == pdata->lcd_ce_mode_store(hisifd->pdev, LCD_CE_MODE_GALLERY, sizeof(LCD_CE_MODE_GALLERY))) {
					HISI_FB_ERR("[effect] lcd_ce_mode_store failed\n");
				}
			}
		}
	...
	HISI_FB_INFO("[effect] ce_mode change to %d\n", ce_ctrl->ctrl_ce_mode);
...
ssize_t hisifb_display_effect_metadata_ctrl_store(struct fb_info *info, const char *buf, size_t count)
{
	...
	if (buf[GALLERY_CTRL_BIT] != GALLERY_OFF && metadata_ctrl->metadata[GALLERY_CTRL_BIT] == GALLERY_OFF) {
		...
		up(&hisifd->blank_sem);
		hisifb_display_effect_ce_ctrl_store(info, EFFECT_CE_GALLERY, sizeof(EFFECT_CE_GALLERY));
	}
...
bool hisifb_display_effect_is_need_blc(struct hisi_fb_data_type *hisifd)
{
	...
	return g_enable_effect & ENABLE_EFFECT_BL && hisifd->bl_enable_ctrl.ctrl_bl_enable == 1 &&
		...
		hisifd->panel_info.hiace_param.iDoAPLC != 0);
}
...
int hisi_effect_arsr1p_config(struct hisi_fb_data_type *hisifd, dss_overlay_t *pov_req)
{
	...
	if ((pov_req->res_updt_rect.w != pinfo->xres)
		...
		post_scf->skin_thres_y = set_bits32(post_scf->skin_thres_y, 0x200534b, 32, 0);
		post_scf->skin_thres_u = set_bits32(post_scf->skin_thres_u, 0x3330a05, 32, 0);
		post_scf->skin_thres_v = set_bits32(post_scf->skin_thres_v, 0x2aa0c06, 32, 0);
		post_scf->skin_expected = set_bits32(post_scf->skin_expected, 0x917196, 32, 0);
