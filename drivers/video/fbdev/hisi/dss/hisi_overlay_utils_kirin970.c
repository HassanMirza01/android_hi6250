...
int hisi_dss_arsr2p_config(struct hisi_fb_data_type *hisifd, dss_layer_t *layer, dss_rect_t *aligned_rect, bool rdma_stretch_enable)
{
	...
	if ((src_width < ARSR2P_MIN_INPUT) || (dst_rect.w < ARSR2P_MIN_INPUT) ||
		...
		if ((!en_hscl) && (!en_vscl) && sharpen_en) {
			sharpen_en = false;
			HISI_FB_INFO("src_rect.w(%d) or dst_rect.w(%d) is smaller than 16 or larger than 2560, sharpen_en bypass!\n",
				src_width, dst_rect.w);
		} else {
...
void hisifb_dss_disreset(struct hisi_fb_data_type *hisifd)
{
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970) {
		media_crg_base = hisifd->media_crg_base;
	} else {
		media_crg_base = hisifd->media_crg_es_base;
	}
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970) {
		...
		outp32(media_crg_base + MEDIA_PEREN0, 0x2009C000);
		outp32(media_crg_base + MEDIA_PEREN1, 0x01E66000);
		outp32(media_crg_base + MEDIA_PEREN2, 0x0000003F);
		udelay(1);

		//3: module clk disable
		outp32(media_crg_base + MEDIA_PERDIS0, 0x2009C000);
		outp32(media_crg_base + MEDIA_PERDIS1, 0x01E06000);
		outp32(media_crg_base + MEDIA_PERDIS2, 0x0000003F);
		udelay(1);

		//4: module unrst
		outp32(media_crg_base + MEDIA_PERRSTDIS0, 0x001C40C0);
		outp32(media_crg_base + MEDIA_PERRSTDIS1, 0x000000F0);

		//5: module clk enable
		outp32(media_crg_base + MEDIA_PEREN0, 0x2009C000);
		outp32(media_crg_base + MEDIA_PEREN1, 0x01E06000);
		outp32(media_crg_base + MEDIA_PEREN2, 0x0000003F);
		...
		//DSI pu
		outp32(peri_crg_base + PEREN5, 0xFFFFFFFF);
		outp32(peri_crg_base + PERRSTDIS3, 0xFFFFFFFF);
	} else {
		///////////////////////////////////////////////////////////////////////////
		//
		//MEDIA_SUBSYS_PU
		...
		//2: module clk enable
		outp32(peri_crg_base + PEREN6, 0x38002020);
		outp32(sctrl_base + SCPEREN4, 0x00000040);
		outp32(peri_crg_base + PEREN4, 0x00000040);
		udelay(1);

		//3: module clk disable
		outp32(peri_crg_base + PERDIS6, 0x38002020);
		outp32(sctrl_base + SCPERDIS4, 0x00000040);
		outp32(peri_crg_base + PERDIS4, 0x00000040);
		udelay(1);
		...
		//2: module clk enable
		outp32(media_crg_base + MEDIA_CLKDIV9, 0x00080008);
		outp32(media_crg_base + MEDIA_PEREN0, 0x080400C0);
		udelay(1);

		//3: module clk disable
		outp32(media_crg_base + MEDIA_PERDIS0, 0x000400C0);
		udelay(1);

		//6: module unrst
		outp32(media_crg_base + MEDIA_PERRSTDIS0, 0x00000030);

		//7: module clk enable
		outp32(media_crg_base + MEDIA_PEREN0, 0x000400C0);
		...
		//
		//2: module clk enable
		outp32(media_crg_base + MEDIA_CLKDIV9, 0x02800280);
