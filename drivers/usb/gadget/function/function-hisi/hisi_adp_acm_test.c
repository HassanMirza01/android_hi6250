...
static int write_async(void *handler, char *buf, unsigned size)
{
	...
	rw_info.pPhyAddr = (char *)virt_to_phys(rw_info.pVirAddr);
	flush_cache_all();
...
int adp_acm_test_open(u32 dev_id)
{
	...
	if (ret) {
		...
		LOG(dev_id, "set read cb done %pK\n", adp_acm_dev->read_done_cb);
	...
	if (ret) {
		...
		LOG(dev_id, "set write cb done %pK\n",
				adp_acm_dev->write_done_cb);
	...
	if (ret) {
		FAIL(dev_id, "init_write_infos failed\n");
...
int bsp_usb_st_acm_modem_open(void)
{
	...
	if (IS_ERR(p_modem_handle)) {
		pr_err("bsp_usb_st_acm_modem_open: %pK\n", p_modem_handle);
		return -1;
	}
	...
	if (bsp_acm_ioctl(p_modem_handle, ACM_IOCTL_SET_FREE_CB, modem_write_done_cb)) {
...
void bsp_usb_modem_st_multi_write(unsigned count, char *header, unsigned int len)
{
	unsigned i;
	int ret;
	...
	for (i = 0; i < count; i++) {
		...
		if (ret) {
			pr_err("acm[acm_modem] bsp_usb_modem_st_write fail %d\n", ret);
			break;
		}
	}
}
