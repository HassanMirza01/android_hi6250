...
static int acm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	/* clear the complete function since it may be assigned other interface's
	   callback complete function before */
...
static inline struct balong_acm_instance *
to_balong_acm_instance(struct config_item *item)
{
	return container_of(to_config_group(item), struct balong_acm_instance,
			func_inst.group);
}
...
static ssize_t f_balong_acm_attr_show(struct config_item *item,
				 ...
				 char *page)
{
	struct balong_acm_instance *inst = to_balong_acm_instance(item);
	struct balong_acm_instance_attribute *balong_acm_instance_attr =
		container_of(attr, struct balong_acm_instance_attribute, attr);
	ssize_t ret = 0;

	if (balong_acm_instance_attr->show)
