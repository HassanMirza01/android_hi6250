...
static void i2c_print_controller_reg(struct dw_i2c_dev *dev)
{
	...
	for (i = 0; i <= HISI_DW_IC_TX_ABRT_SOURCE; i += 0x10) {
		/* can't dump  DW_IC_DATA_CMD */
		if (i != HISI_DW_IC_DATA_CMD) {
			printk(KERN_ERR "%08x: %08x, %08x, %08x, %08x\n",
					(unsigned)(controller->mapbase + i), readl(dev->base + i),
					readl(dev->base + i + 4), readl(dev->base + i + 8),
					readl(dev->base + i + 12));
		} else {
			printk(KERN_ERR "%08x: ........, %08x, %08x, %08x\n",
					(unsigned)(controller->mapbase + i),
					readl(dev->base + i + 4), readl(dev->base + i + 8),
					readl(dev->base + i + 12));
		}
	}
...
static void i2c_dw_dma_probe_initcall(struct dw_i2c_dev *dev)
{
	...
	tx_conf.direction = DMA_TO_DEVICE;
...
static void i2c_dw_dma_tx_callback(void *data)
{
	...
	writew(controller->dmacr, dev->base + DW_IC_DMA_CR);
...
static int hs_dw_i2c_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "i2c reset register vir_addr is: %p\n", controller->reset_reg_base);
