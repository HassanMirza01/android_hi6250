...
static void *create_and_get_mt_pdata(struct device_node *dev_node)
{
	...
	rc = of_property_read_u32(dev_node, "cy,edge_algo_flag", &value);
	if (!rc) {
		pdata->edge_algo_flag = value;
		TS_LOG_INFO("%s,pdata->edge_algo_flag is %d\n", __func__, value);
	} else {
	    pdata->edge_algo_flag = 0;
		TS_LOG_ERR("%s,[cy,edge_algo_flag] read fail, rc = %d\n", __func__, rc);
	}

	if (pdata->edge_algo_flag) {
		array_len = of_property_count_strings(dev_node, "cy,edge_para_config");
		if (array_len > 0 && array_len <= 6) {
			for (index = 0; index < array_len; index++) {
				rc =of_property_read_string_index(dev_node,"cy,edge_para_config",index,&edge_para_dts);
				if (rc) {
					TS_LOG_INFO("read index = %d,edge_para_config = %s,rc = %d error,\n",index,edge_para_dts, rc);
					break;
				}
				pdata->edge_config_buf[index] = simple_strtol(edge_para_dts, NULL, 10);
				TS_LOG_INFO("get chip specific edge_config_buf[%d] = %d\n",index, pdata->edge_config_buf[index]);
			}
		}
	}
	...
	if (!rc) {
		...
		TS_LOG_ERR
		    ("%s %d:[cy,x_max] not found,will cause input dev work error,rc = %d\n",
	...
	if (!rc) {
		...
		TS_LOG_ERR
		    ("%s %d:[cy,y_max] not found,will cause input dev work error,rc = %d\n",
		     __func__, __LINE__, rc);
		goto fail_free_pdata;
	}
	...
	if (!rc) {
		...
		TS_LOG_ERR
		    ("%s %d:[cy,p_max] not found,will cause input dev work error,rc = %d\n",
		     __func__, __LINE__, rc);
		goto fail_free_pdata;
	}
	...
	if (!rc) {
		...
		TS_LOG_ERR
		    ("%s %d:[cy,slots_max] not found,will cause input dev work error, rc = %d\n",
		     __func__, __LINE__, rc);
		goto fail_free_pdata;
	}
	/* Required fields */
...
static struct cyttsp5_core_platform_data *create_and_get_core_pdata(struct
								    device_node
								    *core_node)
{
	...
	if (of_find_property(core_node, "cy,irq_gpio", NULL)) {
		pdata->irq_gpio =
		    of_get_named_gpio_flags(core_node, "cy,irq_gpio", 0, NULL);
	...
	if (of_find_property(core_node, "cy,rst_gpio", NULL)) {
		pdata->rst_gpio =
		    of_get_named_gpio_flags(core_node, "cy,rst_gpio", 0, NULL);
	...
	if (!rc) {
		...
		TS_LOG_INFO("%s,pdata->ttda_cap_test_enable = %d.\n",
			__func__, pdata->ttda_cap_test_enable);
	} else {
		pdata->ttda_cap_test_enable = 0;
	}
#ifndef FINAL_RELEASE_MODE
	rc = of_property_read_u32(core_node, "cy,dmd_noise_report_enable", &value);
	if (!rc) {
		pdata->dmd_noise_report_enable = value;
		TS_LOG_INFO("%s:dmd_noise_report_enable = %d\n", __func__,
						pdata->dmd_noise_report_enable);
	} else {
		pdata->dmd_noise_report_enable = 0;
	}
#endif
	...
	rc = of_property_read_u32(core_node, "cy,mmi_test_gen5", &value);
	if (!rc) {
		pdata->mmi_test_gen5 = value;
	} else {
		pdata->mmi_test_gen5 = 0;
	}

	rc = of_property_read_u32(core_node, "cy,pad_flag", &value);
	if (!rc) {
		pdata->pad_flag = value;
	} else {
		pdata->pad_flag = 0;
	}

	rc = of_property_read_u32(core_node, "cy,sleep_fail_retry", &value);
	if (!rc) {
		pdata->sleep_fail_retry = value;
	} else {
		pdata->sleep_fail_retry = 0;
	}
	...
	rc = of_property_read_string(core_node, "tp_test_type",
				     &pdata->tp_test_type);
	if (rc) {
		TS_LOG_INFO
		    ("get device tp test type  not exit,use default value\n");
		pdata->tp_test_type = "Normalize_type:judge_last_result";
	}

	rc = of_property_read_string(core_node, "cy,product_name",
				     &pdata->product_name);
	if (rc) {
	...
	rc = of_property_read_string(core_node, "cy,support_chip_name",
				     &pdata->chip_name);
	if (rc) {
	...
	rc = of_property_read_u32(core_node, "cy,fw_name_depend_lcd", &value);
	if (!rc) {
		pdata->fw_name_depend_lcd = (u8) value;
		TS_LOG_DEBUG("%s,pdata->fw_name_depend_lcd = %d.\n",
			     __func__, pdata->fw_name_depend_lcd);
	} else {
		pdata->fw_name_depend_lcd = 0;
	}

	rc = of_property_read_u32(core_node, "cy,fw_update_retry_enbale", &value);
	if (!rc) {
		pdata->fw_update_retry_enbale = (u8) value;
		TS_LOG_INFO("%s,fw_update_retry_enbale = %d\n",
				__func__, pdata->fw_update_retry_enbale);
	} else {
		pdata->fw_update_retry_enbale = 0;
	}
	...
	TS_LOG_INFO("%s: "
		    "product_name:%s chip_name:%s irq_gpio:%d rst_gpio:%d"
		    ...
		    __func__, pdata->product_name, pdata->chip_name,
		    pdata->irq_gpio, pdata->rst_gpio, pdata->hid_desc_register,
		    pdata->level_irq_udelay, pdata->vendor_id,
		    pdata->product_id, pdata->flags, pdata->easy_wakeup_gesture,
		    pdata->dtz_x0, pdata->dtz_x1, pdata->dtz_y0, pdata->dtz_y1,
		    pdata->easy_wakeup_supported_gestures,
		    pdata->double_tap_enabled, pdata->upgrade_ttconfig,
		    pdata->has_touchplus, pdata->has_holster,
		    pdata->mmi_test_support);
...
int cyttsp5_get_lcd_panel_info(char *lcd_panel_info)
{
	...
	dev_node = of_find_compatible_node(NULL, NULL, DTS_COMP_LCD_PANEL_TYPE);
	if (!dev_node) {
	...
	strncpy(lcd_panel_info, lcd_type, LCD_PANEL_INFO_MAX_LEN);
		...
		for_each_child_of_node(core_node, dev_node) {
			...
			*pdata_ptr[type].pdata
			...
			if (IS_ERR(*pdata_ptr[type].pdata)) {
				...
				TS_LOG_ERR
				    ("%s,create_and_get_device_pdata fail, rc = %d\n",
				     __func__, rc);
			}
