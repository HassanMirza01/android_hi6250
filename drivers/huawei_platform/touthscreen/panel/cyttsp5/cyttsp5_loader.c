...
#define LCD_INFO_PARTITION_SYMBOL		','
#define LCD_NAME_PARTITION_SYMBOL		'_'
static char g_lcd_panel_info[LCD_PANEL_INFO_MAX_LEN + 1] = {0};
...
static inline struct cyttsp5_loader_data *cyttsp5_get_loader_data(struct device
								  *dev)
{
...
struct cyttsp5_fw_info {
	...
	u8 *chip_name;
	u8 panel_id;
	u8 flag;/*0:default, 1: fw name depend lcd and tp*/
	u8 *id2str;
	u8 *fw_name;
};

static struct cyttsp5_fw_info hw_fw_info[] = {
	{"x1s", "tma568", 0, 0, "oflim", "ts/x1s_TMA568_default.bin"},
	{"x1s", "tma568", 1, 0, "lens", "ts/x1s_TMA568_default.bin"},
	{"Gemini", "tma568", 0, 0, "oflim", "ts/Gemini_TMA568_default.bin"},
	{"Gemini", "tma568", 1, 0, "lens", "ts/Gemini_TMA568_default.bin"},
	{"Alice", "CS445A", 0, 0, "default", "ts/Alice_CS445A_default.bin"},
	{"Alice", "CS445A", 2, 0, "default", "ts/Alice_CS445A_default.bin"},
	{"Alice", "CS445A", 5, 0, "default", "ts/Alice_CS445A_default.bin"},
	{"mozart", "CS448", 0, 0, "ofilm", "ts/mozart_CS448_ofilm.bin"},
	{"mozart", "CS448", 1, 0, "truly", "ts/mozart_CS448_truly.bin"},
	{"Nemo", "CS445A", 0, 0, "oflim", "ts/Nemo_CS445A_default.bin"},
	{"Nemo", "CS445A", 18, 0, "toptouch", "ts/Nemo_CS445A_default.bin"},
    {"Nemo", "CS445A", 5, 0, "junda", "ts/Nemo_CS445A_junda.bin"},
    {"Nemo", "CS445A", 2, 0, "junda", "ts/Nemo_CS445A_junda.bin"},
	{"BTV", "CS448", 0, 0, "ofilm", "ts/BTV_CS448_default.bin"},
	{"BTV", "CS448", 1, 0, "eely", "ts/BTV_CS448_default.bin"},
	{"berlin", "CYTT21403", 0, 0, "oflim", "ts/berlin_CYTT21403_default.bin"},
	{"berlin", "CYTT21403", 6, 1, "lens", "ts/berlin_CYTT21403_default.bin"},
	{"berlin", "CYTT21403", 1, 0, "eely", "ts/berlin_CYTT21403_default.bin"},
	{"prague", "CYTT21403", 0, 0, "oflim", "ts/prague_CYTT21403_default.bin"},
	{"prague", "CYTT21403", 1, 0, "eely", "ts/prague_CYTT21403_default.bin"},
	{"prague", "CYTT21403", 2, 0, "truly", "ts/prague_CYTT21403_default.bin"},
};
					  ...
					  u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	TS_LOG_INFO("%s: Builtin fw version:0x%04X  IC fw version:0x%04X\n",
		    __func__, fw_ver_img, fw_ver_new);
	...
	if (FW_UPDATE_DIFF == cd->cpdata->fw_update_logic) {
		TS_LOG_INFO
		    ("%s: update logic is update when image ver different with IC\n",
		     __func__);
	} else {
	...
	if (FW_UPDATE_DIFF == cd->cpdata->fw_update_logic) {
		if (fw_ver_new != fw_ver_img) {
			TS_LOG_INFO
			    ("%s: Image ver is different, will upgrade\n",
			     __func__);
			return 1;
		}
	} else {
...
exit:
	...
	wake_unlock(&cd->fw_upgrade_lock);
	return rc;
}
...
static int cyttsp5_check_firmware_version_platform(struct device *dev, struct cyttsp5_touch_firmware
						   *fw)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	u32 fw_ver_new;
...
static int upgrade_firmware_from_platform(struct device *dev, bool forced)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	struct cyttsp5_touch_firmware *fw;
	int rc = -ENOSYS;
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_FW_UPGRADE
static int _cyttsp5_firmware_cont(const struct firmware *fw, void *context)
{
	int rc = 0;
						  ...
						  const struct firmware *fw)
{
	...
	upgrade =
	    cyttsp5_check_firmware_version(dev, fw_ver_new, fw_revctrl_new);
					   ...
					   void *context)
{
	...
	int upgrade = 0;
	int rc = 0;
	static int upgrade_retry_count = 0;
	...
	if (!fw) {
		...
		TS_LOG_INFO
		    ("%s: No builtin firmware image found, update skiped!\n",
		     __func__);
	...
	if (true == cd->config_crc_fail_flag) {
		...
		upgrade = 1;
		TS_LOG_INFO
			("%s: FW info is NULL, tp fw may be corrupted, will force upgrade bin\n",
		     __func__);
	}else{
	...
	if (upgrade) {
		rc = _cyttsp5_firmware_cont(fw, dev);
		if (rc && cd->pdata->core_pdata->fw_update_retry_enbale &&
			!upgrade_retry_count) {
			TS_LOG_ERR("%s:do fw updrade fail,try again\n", __func__);
			schedule_work(&ld->fw_and_config_upgrade);
		}

		if (upgrade_retry_count){
#if defined (CONFIG_HUAWEI_DSM)
			if (!dsm_client_ocuppy(tp_cyp_dclient)) {
				cyttsp5_dsm_record_basic_err_info(dev);
				dsm_client_record(tp_cyp_dclient,
						  "fw update failed,retry called,\n retry retval is %d\n", rc);
...
static int upgrade_firmware_from_class(struct device *dev)
{
	...
	retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_NOHOTPLUG,
					 CY_FW_MANUAL_UPGRADE_FILE_NAME, dev,
					 GFP_KERNEL, dev,
					 _cyttsp5_firmware_cont);
	if (retval < 0) {
		TS_LOG_ERR
		    ("%s: Fail request firmware class file load, ret = %d\n",
		     __func__, retval);
		return retval;
	}
...
/* lcd_panel_info format is "hisilicon,auo_ili9885_5p5_1080p_video" */
int cyttsp5_get_lcd_panel_name(struct cyttsp5_core_data *cd)
{
	int num =0, len = 0;
	char *cur = NULL, *next = NULL, *head = NULL;

	if(cyttsp5_get_lcd_panel_info(g_lcd_panel_info)){
		TS_LOG_ERR("%s: get lcd panel info faile!\n ",__func__);
		return -1;
	}

	cur = strchr(g_lcd_panel_info, LCD_INFO_PARTITION_SYMBOL);
	if(!cur){
		TS_LOG_ERR("%s: lcd_panel_info format erron\n ",__func__);
		return -1;
	}
	/*jump partition symbol, points lcd name*/
	head = ++cur;

	while((next = strchr(cur, LCD_NAME_PARTITION_SYMBOL)) != NULL){
		/* Find sencond partition symbol exit */
		if(++num > 1)
			break;
	...
	if(next == NULL){
		TS_LOG_ERR("%s:  get lcd name fail ! , g_lcd_panel_info =%s \n ",
			__func__, g_lcd_panel_info);	
		return -1;
	}

	/* Count lcd panel name length */
	len = next - head;
	/*Copy lcd_panel_name from sencond char from panel_info */
	strncpy(cd->lcd_panel_name, head, len);
	TS_LOG_INFO("%s: lcd_panel_name = %s\n", __func__, cd->lcd_panel_name);
...
static char *generate_firmware_filename(struct device *dev)
{
	...
	if (!filename) {
		TS_LOG_ERR("%s: kzalloc filename fail\n", __func__);
		return NULL;
	}
	...
	if(core_pdata->fw_name_depend_lcd == 1){
		if(!cyttsp5_get_lcd_panel_name(cd)){
			TS_LOG_INFO("%s: get lcd_panel_name succ .\n", __func__);
		}else{
#if defined (CONFIG_HUAWEI_DSM)
			if (!dsm_client_ocuppy(tp_cyp_dclient)) {
				cyttsp5_dsm_record_basic_err_info(cd->dev);
				dsm_client_record(tp_cyp_dclient,
						  "%s: get lcd panel name fail, lcd panel info is:%s\n",
						  __func__, g_lcd_panel_info);
				dsm_client_notify(tp_cyp_dclient,
						  DSM_TP_GET_LCD_PANEL_NAME_ERROR_NO);
			}
#endif
			TS_LOG_ERR("%s: get lcd panel name fail, lcd panel info is:%s\n", __func__,
					g_lcd_panel_info);
			kfree(filename);
			filename = NULL;
		...
		for (index = 0; index < length; index++) {
			retval = strncasecmp(hw_fw_info[index].product_name,
		 				product_name, strlen(hw_fw_info[index].product_name));
			if ((retval == 0) && (hw_fw_info[index].panel_id == panel_id)) {
				flag = hw_fw_info[index].flag ;
				if(flag){
					panel_name = hw_fw_info[index].id2str;
					TS_LOG_INFO("%s: panel_name = %s \n", __func__, panel_name);
				}
				break;
			}
		}
		...
		if(flag){
			snprintf(filename, FILENAME_LEN_MAX, "ts/%s_%s_%s.bin",
			 	product_name, cd->lcd_panel_name, panel_name);
		}else{
			snprintf(filename, FILENAME_LEN_MAX, "ts/%s_%s.bin",
			 	product_name, cd->lcd_panel_name);
		}
	}else{
		length = sizeof(hw_fw_info) / sizeof(struct cyttsp5_fw_info);
		for (index = 0; index < length; index++) {
			retval =
			    strncasecmp(hw_fw_info[index].product_name,
					product_name,
					strlen(hw_fw_info[index].product_name));
			if (retval == 0
			    && hw_fw_info[index].panel_id == panel_id) {
				snprintf(filename, FILENAME_LEN_MAX, "%s",
					 hw_fw_info[index].fw_name);
				findfirmware = true;
				break;
			}
		}
		if (findfirmware == false) {
			snprintf(filename, FILENAME_LEN_MAX, "ts/%s_%s_default.bin",
				product_name, chip_name);
			TS_LOG_INFO
				("%s: no matching firmware, use default firmware: %s\n",
				__func__, filename);
		}
	}
					  ...
					  int ttconfig_size)
{
	...
	if (0) {
		...
		if (rc) {
			TS_LOG_INFO
			    ("%s: crc check fail, will not upgrade ttconfig\n",
			     __func__);
			return 0;
		}
...
static int cyttsp5_check_ttconfig_version_platform(struct device *dev, struct cyttsp5_touch_config
						   *ttconfig)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	u32 fw_ver_config;
	...
	if (cyttsp5_check_firmware_version
	    (dev, fw_ver_config, fw_revctrl_config)) {
		TS_LOG_ERR
		    ("%s: FW and ttconfig versions mismatch, will not upgrade\n",
		     __func__);
		return 0;
	}
...
static struct cyttsp5_touch_config *cyttsp5_get_platform_ttconfig(struct device
								  *dev)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	...
	while ((ttconfig = *ttconfigs++)) {
		...
		TS_LOG_INFO
		    ("%s: ttconfig->product_name = %s,product_name = %s.\n",
...
static int upgrade_ttconfig_from_platform(struct device *dev)
{
	...
	if (upgrade) {
		...
		rc = cyttsp5_upgrade_ttconfig(dev, param_regs->data,
					      param_regs->size);
...
static void cyttsp5_fw_and_config_upgrade(struct work_struct
...
static int cyttsp5_fw_update_from_sdcard(struct device *dev, u8 *file_path,
					 bool update_flag)
{
    int retval;
    u8 *fw_data = NULL;
	...
	if (ld->si) {
		upgrade =
		    cyttsp5_check_firmware_version(dev, fw_ver_new,
						   fw_revctrl_new);
	}
	...
	if (update_flag == 1) {
		...
		if (upgrade > 0) {
			retval = cyttsp5_upgrade_firmware(dev, (const u8 *)
							  &fw_data[header_size +
								   1],
							  fw_size -
							  (header_size + 1));
					  ...
					  const char *buf, size_t size)
{
	...
	rc = cyttsp5_fw_update_from_sdcard(dev, CYTTSP5_FW_FROM_SD_NAME,
					   update_flag);
	if (rc < 0) {
...
static int __init cyttsp5_loader_init(void)
{
	...
	if (cyttsp5_is_init_thread_enable()) {
		TS_LOG_INFO("%s: will init in thread\n", __func__);
		return 0;
	}
