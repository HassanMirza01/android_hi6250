					     ...
					     *hid_output)
{
	...
	if (IS_TMO(t)) {
		...
		if(hid_output->command_code == HID_OUTPUT_NULL) {
			rc = -ETIME;
			TS_LOG_ERR("%s: send HID_OUTPUT_NULL cmd execution timed out\n",
			   __func__);
			goto error;
		}
...
static int cyttsp5_put_device_into_deep_sleep_(struct cyttsp5_core_data *cd)
{
	...
	if (rc) {
		rc = -EBUSY;
		if (cd->pdata->core_pdata->sleep_fail_retry) {
...
static void inline process_charger_armor_level(struct cyttsp5_core_data *cd)
{
	...
	if (cd->last_charger_armor_level != charger_armor_level) {
		TS_LOG_INFO("%s: charger armor level changed : %d to %d\n",
			__func__, cd->last_charger_armor_level,
			charger_armor_level);
#ifndef FINAL_RELEASE_MODE
		if (cd->pdata->core_pdata->dmd_noise_report_enable &&
			!work_pending(&cd->watchdog_work))
				schedule_work(&cd->noise_report_work);
#endif
	}
...
#ifndef FINAL_RELEASE_MODE
void cyttsp5_noise_report_work(struct work_struct *work)
{
#if defined (CONFIG_HUAWEI_DSM)
	struct cyttsp5_core_data *cd =
		container_of(work, struct cyttsp5_core_data, noise_report_work);
	u8 current_noise_level = cd->last_charger_armor_level;
	...
	if (!dsm_client_ocuppy(tp_cyp_dclient)) {
		dsm_client_record(tp_cyp_dclient,
				...
				"panel_id:%d\n"\
				"lcd_panel:%s\n",
				current_noise_level, charger_type,
				cd->sysinfo.cydata.fw_ver_conf, cd->panel_id,
				cd->lcd_panel_name);
		dsm_client_notify(tp_cyp_dclient, DSM_TP_NOISE_INFO_REPORT_ERROR_NO);
		TS_LOG_INFO("%s:dsm report touch noise level:%d,charger_type=%d\n",
				__func__, current_noise_level, charger_type);
	}
#endif
}
#endif
...
static void cyttsp5_panel_id_work_function(struct work_struct *work)
{
	struct cyttsp5_core_data *cd = container_of(work,
						    struct cyttsp5_core_data,
						    panel_id_work);
	int panel_id = cd->panel_id;

	TS_LOG_DEBUG("%s: panel_id = %d\n", __func__, panel_id);
	/*call notify list*/
		  ...
		  u16 irq, size_t xfer_buf_size)
{
	...
	INIT_WORK(&cd->panel_id_work, cyttsp5_panel_id_work_function);
#ifndef FINAL_RELEASE_MODE
	if (cd->pdata->core_pdata->dmd_noise_report_enable) {
		INIT_WORK(&cd->noise_report_work, cyttsp5_noise_report_work);
		cyttsp5_get_lcd_panel_name(cd);
		TS_LOG_INFO("%s:dmd report noise info enable\n", __func__);
	}
#endif
	...
	TS_LOG_INFO("%s: start panel_id_work\n", __func__);
	schedule_work(&cd->panel_id_work);
	return 0;
...
error_startup:
	cancel_work_sync(&cd->startup_work);
	cancel_work_sync(&cd->panel_id_work);
	cyttsp5_stop_wd_timer(cd);
	cyttsp5_free_si_ptrs(cd);
