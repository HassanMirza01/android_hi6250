...
extern volatile int g_lcd_brightness_info;
...
#define STR_IC_VENDOR "Parade"
#define STR_IC_NAME "TT41701"
#define STR_PROJECT_ID "VICK310800"
#define STR_MOD_VENDOR "JDI"
...
#ifdef PARADE_ROI_ENABLE
	.chip_roi_rawdata = parade_roi_rawdata,/*Not Vefrified*/
		...
		const char *data_name)
{
	...
	u8 *pr_buf = &parade_core_data.pr_buf[0];
	int i, k;
	const char fmt[] = "%02X ";
	int max;
...
static int parade_get_brightness_info(void)
{
	...
	if(parade_core_data.mode == CY_MODE_BOOTLOADER){
...
static int parade_before_suspend(void)
{
	...
	struct parade_core_data *cd = &parade_core_data;
	TS_LOG_INFO("%s Enter", __func__);
	if(cd->isbootupdate_finish==false) {
...
static int parade_core_suspend(void)
{
	...
	if(0 == sleep_in_mode){ //Deep Sleep
		...
		if(parade_core_data.isbootupdate_finish) {
			disable_irq(g_ts_data.irq_id);/*Disable IRQ to avoid the glitch changes of LCD*/
		}
...
static unsigned char * parade_roi_rawdata (void)
{
	...
	cyttsp5_pr_buf(parade_core_data.knuckle_data, PARADE_ROI_DATA_SEND_LENGTH, "knucle_data");
...
static int parade_set_roi_switch(u8 roi_switch)
{
	...
	if(roi_switch == 0) {
		...
		rc = parade_stop_knuckle_mode(&parade_core_data);
		if(!rc)
			parade_core_data.roi_switch = 0;
		else
			TS_LOG_ERR("%s stop knuncle report mode failed\n", __func__);
		TS_LOG_INFO("%s turn off knuncle report mode,rc=%d\n", __func__, rc);
	} else {
		/*in case of turn on the heatmap tracking mode*/
		rc = parade_start_knucle_mode(&parade_core_data);
		if(!rc)
			parade_core_data.roi_switch = 1;
		else
			TS_LOG_ERR("%s start knuncle report mode failed\n", __func__);
		TS_LOG_INFO("%s turn on knuncle report mode, rc=%d\n", __func__, rc);
	}
exit:
...
static int parade_init_chip(void)
{
	...
	parade_core_data.isbootupdate_finish = true;
	parade_core_data.parade_chip_data->isbootupdate_finish = parade_core_data.isbootupdate_finish;
...
err_pinctrl_put:
 ...
 static int parade_parse_dts(struct device_node *device, struct ts_device_data *chip_data)
 {
	...
	g_ts_data.feature_info.roi_info.roi_supported = 1;//TODO: ONLY FOR TEST USE
	...
	parade_core_data.parade_chip_data->should_check_tp_calibration_info = 0; //TODO: TEST ONLY// = parade_get_dts_value(core_node, "should_check_tp_calibration_info");
	...
	if(value == 1) {
		...
		TS_LOG_INFO("%s, enable is_parade_solution \n", __func__);
	}else{
	...
	if(value == 1) {
		...
		TS_LOG_INFO("%s, enable direct_proc_cmd \n", __func__);
	}else{
...
{
#ifdef PARADE_ROI_ENABLE
	if(parade_core_data.roi_switch != 0) {
...
static int parade_load_app_(struct device *dev, const u8 *fw, int fw_size)
{
	...
	if(parade_core_data.mode == CY_MODE_OPERATIONAL){
...
static int parade_fw_update_boot(char *file_name)
{
	...
	parade_core_data.isbootupdate_finish = false;
	parade_core_data.parade_chip_data->isbootupdate_finish = parade_core_data.isbootupdate_finish;
	parade_loader_probe(&parade_core_data);
	...
	if(rc == 0){
		TS_LOG_INFO("%s: FW upgrade done, Restart up after fw upprade\n",
					__func__);
		rc = parade_startup(&parade_core_data,false);
		if(rc)
...
static int parade_check_firmware_version(u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	if(!parade_core_data.sysinfo.ready){
...
static int parade_check_cmd_status()
{
	...
	int timeout_ms = PARADE_WORK_WAIT_TIMEOUT;
	TS_LOG_INFO("%s Enter\n", __func__);
	t = wait_event_timeout(parade_core_data.wait_q, (parade_core_data.isworkongoing == false), msecs_to_jiffies(timeout_ms));
		if (IS_TMO(t)) {
			TS_LOG_ERR("%s: tmo waiting access work, t = %d\n",
				   __func__, t);
			return -EBUSY;
	}
	parade_core_data.isworkongoing = true;
	return NO_ERR;
}

static void parade_finish_cmd()
{
	parade_core_data.isworkongoing = false;
	wake_up(&parade_core_data.wait_q);
}
...
static int parade_chip_get_info(struct ts_chip_info_param *info)
{
	...
	snprintf(info->ic_vendor, sizeof(info->ic_vendor), STR_IC_VENDOR);
	snprintf(info->chip_name, sizeof(info->chip_name), STR_IC_NAME);
	snprintf(info->mod_vendor, sizeof(info->mod_vendor), STR_MOD_VENDOR);
	parade_finish_cmd();
	return retval;
}
...
exit:
	if(strlen(buf_parademmitest_result) != 0)
	{
		...
		AddtoMMIResult("-");
		AddtoMMIResult(STR_IC_VENDOR);
		AddtoMMIResult("-");
		AddtoMMIResult(STR_IC_NAME);
		AddtoMMIResult("-");
		AddtoMMIResult(STR_PROJECT_ID);
...
void ts_check_bootup_upgrade(void)
{
	int max_timeout = 100000;//10s
	int check_peoriod = 50;
	int iCount = max_timeout/check_peoriod;
	TS_LOG_INFO("%s enter, with max timeout: %d s\n", __func__,max_timeout/1000);
	while(iCount>0){
		if(parade_core_data.isbootupdate_finish){
			break;
		}
		...
		{
			msleep(check_peoriod);
			iCount--;
		}
	}
	TS_LOG_INFO("%s exit with timeout: %d\n", __func__,iCount);
}
EXPORT_SYMBOL(ts_check_bootup_upgrade);
