...
static int32_t hideep_get_vreg(struct ist510e *ts, u32 addr, u32 len)
{
	...
	return ret;
i2c_err:TS_LOG_ERR("hideep_get_vreg error\n");
	return ret;
}
...
static int32_t hideep_set_vreg(struct ist510e *ts, u32 addr, u32 len)
{
	...
	return ret;
i2c_err:TS_LOG_ERR("hideep_set_vreg error\n");
	return ret;
}

static size_t hideep_download_uc(struct ist510e *ts, const char __user *uc,
				    int offset, size_t count)
{
	...
	TS_LOG_INFO("start offset = 0x%0x, count = %zu\n", offset, count);
	if (offset>64*1024 || count>64*1024){
	...
	kfree(ucode);
	TS_LOG_INFO("ist510e_download_uc(%zu)\n", count);
	return count;
hideep_download_uc_err:TS_LOG_ERR("data(size = %zu) to be downloaded is oversize \n",
		(offset) + count);
	return -1;
}
...
static unsigned int hideep_iface_poll(struct file *file,
...
static ssize_t hideep_iface_read(struct file *file, char __user *buf,
				size_t count, loff_t *offset)
{
	...
	u16 *p_z;
	TS_LOG_DEBUG
	("hideep_iface_read start count = %zu, (*offset) = %ld(0x%lx)\n",
	count, *offset, *offset);
	if (!file->private_data) {
		TS_LOG_ERR("file->private_data is NULL\n");
		return -EFAULT;
	}
	if ((IST_Z_CALIB2 == (*offset))
	&&
	(((drv_info->ts->z_calib_end - drv_info->ts->z_calib_start +
		1) * 2 + 2) == count) && (drv_info->ts->z_flag_ready)) {
		TS_LOG_DEBUG("z_data read, index = %d\n, start = %d, end = %d",
			drv_info->ts->z_index,
			drv_info->ts->z_calib_start,
			drv_info->ts->z_calib_end);
		...
		memcpy(rd_buffer + 2,
			&drv_info->ts->z_data[drv_info->ts->z_calib_start],
			count - 2);
		rd_len = count;
		...
		rd_buffer = drv_info->vr_buff;
		rd_len = count;
		if ((IST_FIRST_Z_VALUE == (*offset)) && (2 == count)
		&& (drv_info->ts->z_status)) {
			...
			*p_z = drv_info->ts->z_buffer;
			rd_len = 2;
			ret = 0;
		} else if ((IST_APPLE_WEIGHT_VALUE == (*offset))
			&& (2 == count)
			&& (drv_info->ts->apple_weight_flag)) {
	...
	return rd_len;
hideep_iface_read_err:return -1;
}

static ssize_t hideep_iface_write(struct file *file, const char __user *buf,
				size_t count, loff_t *offset)
{
	int ret = 0;
	struct ist510e_debug_dev *drv_info = file->private_data;
	...
	int wr_len = 0;
	TS_LOG_DEBUG("hideep_iface_write count = %zu, (*offset) = %ld(0x%lx)\n",
			count, *offset, *offset);
	if (!file->private_data) {
		TS_LOG_ERR("file->private_data is NULL\n");
		return -EFAULT;
	...
	if ((*offset) < ADDR_VR_END) {
		wr_len =
		(count < IST_VR_SIZE_LENGTH) ? count : IST_VR_SIZE_LENGTH;
		...
		if ((VR_SINGLE_TOUCH == (*offset)) && (1 == count)) {
			...
			case 0:
				...
				ret =
				hideep_set_vreg(drv_info->ts, *offset,
						wr_len);
				break;
			case 1:
				if (!drv_info->ts->apple_weight_init) {
					...
					drv_info->ts->apple_weight_timeout = 0;
					if (drv_info->
					ts->apple_weight_workqueue)
						queue_work(drv_info->
							ts->apple_weight_workqueue,
							&drv_info->
							ts->apple_weight_work);
				} else {
			...
			default:
				TS_LOG_ERR
				("error apple weight incorrect parameter\n");
				break;
			}
			return 1;
		} else
		...
		TS_LOG_INFO("download fw vr, offset = 0x%lx, count = %zu\n",
			(*offset), count);
		wr_len =
		hideep_download_uc(drv_info->ts, buf, (*offset) & 0xffff,
				count);
	} else {
...
static loff_t hideep_iface_llseek(struct file *file, loff_t off, int whence)
{
	loff_t newpos = 0;
	struct ist510e_debug_dev *drv_info = file->private_data;
	TS_LOG_INFO("start, whence = %d, offset = 0x%08lx\n", whence, off);
	if (!file->private_data) {
		TS_LOG_ERR("file->private_data is NULL\n");
		return -EFAULT;
	}
	...
	case 1:
		TS_LOG_INFO("hideep_iface_llseek set mode, off = 0x%08lx\n",
			     off);
		if (off == 0x1000) {
			drv_info->im_r_en = 1;
			...
			TS_LOG_DEBUG
			("hideep_iface_llseek set release flag \n");
			...
			if ((drv_info->ts->z_calib_end -
			drv_info->ts->z_calib_start + 1) > (1024 * 4)) {
				TS_LOG_ERR
				("hideep_iface_llseek set the frames is oversize\n");
				return -EINVAL;
			}
			TS_LOG_DEBUG
			("hideep_iface_llseek set calib2 start = %d, end = %d\n",
			drv_info->ts->z_calib_start,
			drv_info->ts->z_calib_end);
	...
	case 2:
		TS_LOG_INFO("set im/vr size, off = 0x%08lx\n", off);
...
static long hideep_iface_ioctl(struct file *file, unsigned int cmd,
				unsigned long arg)
{
struct ist510e_debug_dev *drv_info = file->private_data;
	int err = 0;
	...
	TS_LOG_INFO("cmd = 0x%8x,IST510E_CFG = 0x%8x,IST510E_RDIM = 0x%8x\n",
		cmd, (unsigned int)IST510E_CFG,
		(unsigned int)IST510E_RDIM);
	...
	case IST510E_CFG:
		ret =
		   copy_from_user(&config, (void *)arg,
				sizeof(struct ist510e_debug_cfg));
...
static int hideep_iface_register(struct ist510e *ts, u32 minor)
{
	...
	device = device_create(ts->debug_class, NULL,
			devno, NULL, ISTCORE_IF_DEVICE_NAME);
	if (IS_ERR(device)) {
		err = PTR_ERR(device);
		cdev_del(&dev->cdev);
		goto err;
