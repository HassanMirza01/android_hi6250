...
void dhd_enable_packet_filter(int value, dhd_pub_t *dhd)
{
	...
	if (dhd_pkt_filter_enable && (!value ||
		...
		dhd_iovar(dhd, 0, "pkt_filter_mdns", (char *)&value, sizeof(value), 1);
#endif /* PKT_FILTER_SUPPORT */
	}
#endif /* PKT_FILTER_SUPPORT */
}
...
static int dhd_ant_init(dhd_pub_t *dhd)
{
	...
	ant_event_param.thresh_low_core1 = htod32(-88);
	...
	ant_event_param.hyst_time_high = htod32(2);

	err = dhd_iovar(dhd, 0, "ant_event", (char *)&ant_event_param, sizeof(wl_ant_qual_event_t), 1);
	if (err == BCME_UNSUPPORTED) {
		DHD_ERROR(("Current firmware doesn't support antenna event\n"));
	}
	if (err != BCME_OK)
		DHD_ERROR(("ljy: dhd_ant_init failed, err: %d", err));
	return err;
}
#endif
...
int dhd_dev_cfg_rand_mac_oui(struct net_device *dev, uint8 *oui)
{
	...
	if (ETHER_ISMULTI(oui)) {
		...
		err = dhd_iovar(dhdp, 0, "scan_macaddr", (char *)&cfg, sizeof(cfg), 1);
		if (err < 0) {
