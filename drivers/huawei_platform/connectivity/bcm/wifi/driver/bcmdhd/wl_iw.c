...
#define IW_MAX_INPUT_SIZE                 4 * 1024
#endif
...
#ifdef HW_WIFI_SECURITY_PATCH
	if (wrqu->data.length != 0 && wrqu->data.length <= IW_MAX_INPUT_SIZE) {
#else
...
#ifdef HW_WIFI_SECURITY_PATCH
		WL_ERROR(("IWPRIV argument len = %d \n", wrqu->data.length));
#else
        ...
        char *ext)
{
	...
	WL_SOFTAP((">>%s called\n args: info->cmd:%x,"
		...
		wrqu->data.pointer, wrqu->data.length));
#ifdef HW_WIFI_SECURITY_PATCH
	if (wrqu->data.length != 0 && wrqu->data.length < 256) {
#else
	if (wrqu->data.length != 0) {
#endif
       ...
       int sar_enable)
{
       ...
       } else if(sar_enable == SAR_ENABLE) {
		error = dev_wlc_intvar_set(dev, "sar_enable", 1);
		WL_ERROR(("%s enter sar_enable  \n",__FUNCTION__));
       } else {
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	case WLC_E_REASSOC_IND:
		...
		if  (strstr(g_fw_path, "apsta") != NULL &&
			...
			kobject_uevent_env(&(priv_dev->dev.kobj), KOBJ_CHANGE, env.envp);
#else
