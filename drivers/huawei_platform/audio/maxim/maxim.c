...
struct dsm_client *smartpa_dclient;
#endif
...
#ifdef CONFIG_HUAWEI_DSM
	if (ret && !dsm_client_ocuppy(smartpa_dclient)) {
		dsm_client_record(smartpa_dclient, "%s: maxim open error %d\n", __func__, ret);
		dsm_client_notify(smartpa_dclient, DSM_SMARTPA_I2C_ERR);
	}
#endif
...
#ifdef CONFIG_HUAWEI_DSM
	if (ret && !dsm_client_ocuppy(smartpa_dclient)) {
		dsm_client_record(smartpa_dclient, "%s: ioctl error %d\n", __func__, ret);
		dsm_client_notify(smartpa_dclient, DSM_SMARTPA_I2C_ERR);
	}
#endif
	return ret;
...
#ifdef CONFIG_HUAWEI_DSM
	if (!smartpa_dclient) {
		smartpa_dclient = dsm_register_client(&dsm_smartpa);
	}
#endif
