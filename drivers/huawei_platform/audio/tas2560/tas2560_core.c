...
{
	/* reg address			size	values */
	TAS2560_BOOST_ON,		0x04,	0x75, 0xC2, 0x8F, 0x00,
	TAS2560_BOOST_OFF,		0x04,	0x6E, 0x14, 0x7A, 0x00,
	TAS2560_BOOST_TABLE_CTRL1,	0x04,	0x1D, 0x99, 0x99, 0x00,
	TAS2560_BOOST_TABLE_CTRL2,	0x04,	0x20, 0xcc, 0xcc, 0xcc,
	TAS2560_BOOST_TABLE_CTRL3,	0x04,	0x24, 0x00, 0x00, 0x00,
	TAS2560_BOOST_TABLE_CTRL4,	0x04,	0x27, 0x33, 0x33, 0x33,
	TAS2560_BOOST_TABLE_CTRL5,	0x04,	0x2a, 0x66, 0x66, 0x66,
		...
		if (nRegister == TAS2560_MDELAY){
			...
			if (nRegister != 0xFFFFFFFF){
				...
				if(nSize > 1){
					...
					pTAS2560->bulk_write(pTAS2560, nRegister, Buf, nSize);
				}else if(nSize == 1){
					pTAS2560->write(pTAS2560,nRegister, nData[0]);
				}else{
					dev_err(pTAS2560->dev,
...
int tas2560_fadeIn_fadeout(struct tas2560_priv *pTAS2560, int muteflag, unsigned int time)
{
	...
	if((find_reg_val>=0)&&(find_reg_val<MAX_SUPPORTED_FADETYPE)){
		...
		tas2560_mute(pTAS2560,muteflag);
		ret = 0;
	}else{
...
#if 0
	pTAS2560->mnResetGPIO = of_get_named_gpio(np,"gpio_reset", 0);
	if (pTAS2560->mnResetGPIO < 0) {
		dev_err(pTAS2560->dev, "Looking up %s property in node %s failed %d\n",
			"gpio_reset", np->full_name,
			pTAS2560->mnResetGPIO);
		ret = -1;
	}else{
		dev_dbg(pTAS2560->dev, "gpio_reset=%d", pTAS2560->mnResetGPIO);
	}
#else
	rc = of_property_read_u32(np, "gpio_reset", &pTAS2560->mnResetGPIO);
	if (rc) {
		dev_err(pTAS2560->dev, "Looking up %s property in node %s failed %d\n",
			"gpio_reset", np->full_name, rc);
		ret = -1;
	}else{
...
void tas2560_enable(struct tas2560_priv *pTAS2560, bool bEnable)
{
	if (bEnable) {
		if (!pTAS2560->mbPowerUp) {
			...
			/*unmute device */
			pTAS2560->write(pTAS2560,TAS2560_MUTE_REG, 0x40);
