...
*  return value:  NULL
**********************************************************/
static void charge_core_tbatt_handler(int temp,
	...
	for (i = 0; i < VOLT_PARA_LEVEL; i++) {
		if ((vbat >= volt_data[i].vbat_min) && (vbat < volt_data[i].vbat_max)) {
			if ((last_vbat - vbat <= 0)
			    ...
			    || (abs(last_i - i) > 1)
			    || (flag_running_first == 1)) {
				data->iin = data->iin < volt_data[i].iin_volt ?
					data->iin : volt_data[i].iin_volt;
				data->ichg = data->ichg < volt_data[i].ichg_volt ?
					data->ichg : volt_data[i].ichg_volt;
			} else {
				      ...
				      struct charge_core_data *data)
{
    ...
    if (vbat > last_vbat){
        for (i = 0; i < VDPM_PARA_LEVEL; i++){
            if ((vbat >= vdpm_data[i].cap_min)
                && (vbat < vdpm_data[i].cap_max)){
        ...
        for (i = 0; i < VDPM_PARA_LEVEL; i++){
            if (((vbat + vdpm_data[i].cap_back) >= vdpm_data[i].cap_min)
                && ((vbat + vdpm_data[i].cap_back) < vdpm_data[i].cap_max)){
                data->vdpm = vdpm_data[i].vin_dpm;
                break;
            }
        }
						   ...
						   *data)
{
	...
	if (last_i == i || flag_running_first == 1) {
		data->ichg = data->ichg < segment_data[i].ichg_segment ?
			data->ichg : segment_data[i].ichg_segment;
		data->vterm = data->vterm < segment_data[i].vterm_segment ?
			data->vterm : segment_data[i].vterm_segment;
	} else {
		data->ichg = data->ichg < last_ichg ? data->ichg : last_ichg;
		data->vterm = data->vterm < last_vterm ? data->vterm : last_vterm;
	}
	hwlog_info
	    ("%s :ichg = %d,vbat =%d , last_i =%d, i =%d,segment_data[i].ichg_segment =%d ,data->ichg =%d ,last_ichg = %d \n",
	...
	for (i = 0; i < INDUCTANCE_PARA_LEVEL; i++) {
		if ((cbat >= inductance_data[i].cap_min) && (cbat < inductance_data[i].cap_max)) {
			if ((last_i - i <= 0)
			    ...
			    || (abs(last_i - i) > 1)
			    || (flag_running_first == 1)) {
				data->iin = data->iin < inductance_data[i].iin_inductance ?
					data->iin : inductance_data[i].iin_inductance;
			} else {
				data->iin = data->iin < last_iin ? data->iin : last_iin;
			}
			break;
