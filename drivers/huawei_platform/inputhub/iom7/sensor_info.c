...
static struct timeval sensor_power_vddio_time;
...
BH1745_ALS_PARA_TABLE als_para_diff_tp_color_table[] = {
	...
	{492, 530, 485, 2959, 2204, 492, 517, 1667, 100, -2540, 6486, -4010,
	 ...
	 {773,641,444,2909,1706,773,409,1231,100,-2447,4429,-3594,
	 5096,979,1437,4251,1966,3322,2975,3230,467,5931,6445,6000,
	 150}},
...
PINHOLE_ALS_PARA_TABLE pinhole_als_para_diff_tp_color_table[] = {
	{BLN, V4, APDS9922, TS_PANEL_UNKNOWN, {1200, 400, 268, 327, 400}},
	{BLN, V4, APDS9922, TS_PANEL_OFILIM, {1200, 400, 314, 380, 536}},
	{BLN, V4, APDS9922, TS_PANEL_LENS, {1200, 400, 268, 327, 400}},
	{BLN, V4, APDS9922, TS_PANEL_EELY, {1300, 400, 303, 348, 560}},

	{BLN, V4, LTR578, TS_PANEL_UNKNOWN, {1200, 300, 570, 570, 930}},
	{BLN, V4, LTR578, TS_PANEL_OFILIM, {1200, 300, 600, 600, 800}},
	{BLN, V4, LTR578, TS_PANEL_LENS, {1200, 300, 450, 530, 620}},
	{BLN, V4, LTR578, TS_PANEL_EELY, {1200, 300, 570, 570, 930}},

	{PRA, V4, APDS9922, TS_PANEL_UNKNOWN, {980, 350, 306, 366, 558}},
	{PRA, V4, APDS9922, TS_PANEL_OFILIM, {980, 350, 330, 366, 620}},
	{PRA, V4, APDS9922, TS_PANEL_TRULY, {980, 350, 306, 366, 558}},
	{PRA, V4, APDS9922, TS_PANEL_EELY, {980, 350, 310, 390, 656}},

	{PRA, V4, LTR578, TS_PANEL_UNKNOWN, {1150, 350, 600, 600, 724}},
	{PRA, V4, LTR578, TS_PANEL_OFILIM, {1150, 350, 600, 600, 800}},
	{PRA, V4, LTR578, TS_PANEL_TRULY, {1150, 350, 600, 600, 724}},
	{PRA, V4, LTR578, TS_PANEL_EELY, {1150, 350, 480, 560, 900}},
};
...
int detect_i2c_device(struct device_node *dn, char *device_name)
{
	...
	if (pkg_mcu.errno != 0) {
		...
		for (i = 0; i < len; i++) {
			if ((pkg_mcu.data[0] == (char)wia[i]) || ((pkg_mcu.data[0] & CHIP_VERSION_MASK) == (char)wia[i])) {
				hwlog_info
		...
		hwlog_info("%s:i2c detect fail,chip_value:0x%x,len:%d!\n",
			device_name, pkg_mcu.data[0], len);
		return -1;
	}
}
	...
	uint8_t register_add, uint8_t rw, int len, uint8_t *buf)
{
	...
	pkt_i2c_write_req_t *pkt_i2c_write =
	    kzalloc(sizeof(pkt_i2c_write_req_t) + len, GFP_KERNEL);
	if (!pkt_i2c_write) {
	...
	case CMD_I2C_WRITE_ONLY_REQ:
		...
		pkg_ap.wr_len =
		    sizeof(*pkt_i2c_write) - sizeof(pkt_i2c_write->hd) + len;
		break;
	default:
		hwlog_err("rw err:%d\n", rw);
		break;
...
void read_als_data_from_dts(struct device_node *dn)
{
	...
	if (rohm_rgb_flag == 1) {
		if (phone_color != BLACK && phone_color != WHITE
		    && phone_color != GOLD && phone_color != GRAY
		    && phone_color != BROWN && phone_color != PINK
		    && phone_color != CAFE_2 && phone_color != BLACK2) {
			/*use the defoult als para adjust to the tp black color*/
			memcpy(als_data.als_extend_data,
			       als_para_diff_tp_color_table[0].bh745_para,
			       sizeof(als_para_diff_tp_color_table[0].
				      bh745_para));
		} else {
			for (i = 0;
		...
		if (phone_color != BLACK && phone_color != WHITE
			...
			/*use the defoult als para adjust to the tp black color*/
			memcpy(als_data.als_extend_data,
			       apds_als_para_diff_tp_color_table[0].
			       apds251_para,
			       sizeof(apds_als_para_diff_tp_color_table[0].
				      apds251_para) >
			       SENSOR_PLATFORM_EXTEND_DATA_SIZE ?
			       SENSOR_PLATFORM_EXTEND_DATA_SIZE :
			       sizeof(apds_als_para_diff_tp_color_table[0].
			 ...
			 size_t count)
{
	...
	hwlog_info
	    ("In %s! bus_num = %d, i2c_address = %d, reg_add = %d, len = %d, rw = %d, buf_temp[0] = %d\n",
			   ...
			   size_t count)
{
	...
	hwlog_info
	    ("In %s! bus_num=%d, i2c_address=%d, reg_add=%d, len=%d, rw=%d, buf_temp[0]=0x%02x, buf_temp[1]=0x%02x\n",
	     __func__, bus_num, i2c_address, reg_add, len, rw, buf_temp[0],
	     buf_temp[1]);
	ret = mcu_i2c_rw(bus_num, i2c_address, reg_add, rw, len, buf_temp);
	if (ret < 0)
					...
					const char *buf, size_t count)
{
	...
	if ((sensor_status.delay[TAG_ACCEL] == 0)
	    || (sensor_status.delay[TAG_ACCEL] > 100)) {
		delay = 100;
	...
	hwlog_err("acc calibrator val is %d  len is %d.\n", val, sizeof(val));
	ret = write_customize_cmd(&pkg_ap, &pkg_mcu);
	if (ret) {
		return_calibration = COMMU_FAIL;
...
static int rgb_cal_result_write_file(char *filename, char *param)
{
	...
	if (IS_ERR_OR_NULL(fop)) {
		set_fs(old_fs);
		hwlog_err
		    ("Create file error!! Path = %s IS_ERR_OR_NULL(fop) = %ld fop = %p\n",
...
static int als_calibrate_save(const void *buf, int length)
{
	...
	snprintf(buftemp, RGB_SENSOR_CAL_RESULT_MAX_LEN,
		 "%d  %d  %d %d  %d  %d  %d  %d  %d  %d",
		 bh1745_als_cal_result[0], bh1745_als_cal_result[1],
		 bh1745_als_cal_result[2], bh1745_als_cal_result[3],
		 bh1745_als_cal_result[4], bh1745_als_cal_result[5],
		 phone_color, tplcd_manufacture, rohm_rgb_flag,
		 avago_rgb_flag);
				  ...
				  const char *buf, size_t size)
{
	...
	hwlog_info
	    ("val = %d start_update_flag = %d precise = %d interval= %d\n", val,
...
int tell_cts_test_to_mcu(int status)
{
	...
	if ((0 == status) || (status == 1)) {
		...
		return inputhub_mcu_write_cmd_adapter(&pkt, sizeof(pkt), &pkg_mcu);
	} else {
...
{
	int ret = 0, result = 0;
	struct timeval sys_ready_time;
	unsigned long time_of_vddio_power_reset = 0;
#ifdef CONFIG_HUAWEI_CHARGER_SENSORHUB
	enum hisi_charger_type type = hisi_get_charger_type();
#endif

	memset(&sys_ready_time, 0, sizeof(sys_ready_time));
	if (ST_MINSYSREADY == ((pkt_sys_statuschange_req_t *) head)->status) {
		...
		if(need_reset_io_power) {
			do_gettimeofday(&sys_ready_time);
			time_of_vddio_power_reset = (sys_ready_time.tv_sec - sensor_power_vddio_time.tv_sec)*1000 +
				(sys_ready_time.tv_usec)/1000 - (sensor_power_vddio_time.tv_usec)/1000;
			if (time_of_vddio_power_reset < SENSOR_MAX_RESET_TIME_MS)
				msleep(SENSOR_MAX_RESET_TIME_MS - time_of_vddio_power_reset);

			hwlog_info("time_of_vddio_power_reset %lu\n", time_of_vddio_power_reset);
...
static int sensorhub_io_driver_probe(struct platform_device *pdev)
{
	...
	if (!of_match_device(sensorhub_io_supply_ids, &pdev->dev)) {
        ...
        }else
        {
            ...
            hwlog_info("%s property need-reset is %d", __func__, val);
        }
	...
	if(need_reset_io_power) {
		...
		do_gettimeofday(&sensor_power_vddio_time);
	}
