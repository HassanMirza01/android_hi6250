...
#define PERI_USED_TIMEOUT (jiffies + HZ/25)
...
static void __iomem *pmctrl_reg_base;
...
static void peri_used_timeout(unsigned long data)
{
	...
	if(0 == peri_used) {
		writel(0, SOC_PMCTRL_PERI_CTRL6_ADDR(pmctrl_reg_base));
		peri_used_t = 0;
	}
...
void peri_used_init(void)
{
	...
	if (!np) {
		pr_err("[%s]get pmctrl node failed!\n", __func__);
		return;
	}

	pmctrl_reg_base = of_iomap(np, 0);
	if (!pmctrl_reg_base) {
		pr_err("[%s]iomap pmctrl_reg_base failed!\n", __func__);
		return;
	}
...
void peri_used_request(void)
{
	...
	if (0 == peri_used_t)
		writel(0xFFFFFFFF, SOC_PMCTRL_PERI_CTRL6_ADDR(pmctrl_reg_base));
