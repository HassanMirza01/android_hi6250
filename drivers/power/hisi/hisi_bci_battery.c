...
int check_charge_temp_fault(char *buf)
{
	...
	struct hisi_bci_device_info *di = g_hisi_bci_dev;
	int temp = hisi_battery_temperature();
	if ((HIGH_TEMP < temp || LOW_TEMP > temp) && (di->usb_online || di->ac_online)) {
...
int check_charge_batt_capacity(char *buf)
{
	...
	if ((VCHRG_CHARGE_DONE_EVENT == di->event) && full_rm < fcc_design*80/100) {
			snprintf(buf, 1024, "batt_vol = %d,capacity = %d,full capacity = %d \n", di->bat_voltage,di->bat_rm,fcc_design);
			return 1;
	}
	return 0;
}
...
int check_soc_vary_err(char *buf)
{
	...
	if(monitor_cnt % (SOC_MONITOR_INTERVAL / di->monitoring_interval) == 0){
		...
		if(data_invalid){
			...
			snprintf(buf, 1024, "soc change fast: %d\n", deta_soc);
			report_flag = 1;
		}
