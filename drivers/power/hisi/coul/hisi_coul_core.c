...
static struct battery_aging_safe_policy basp_policy[BASP_LEVEL_CNT] = { /* Note:The level is listed descendingly */
...
********************************************************/
int coul_ntc_temperature_compensation(struct smartstar_coul_device *di)
{
    ...
    temp_without_compensation = coul_battery_temperature_tenth_degree();
...
static int get_temperature_stably(struct smartstar_coul_device *di)
{
	...
	while(cnt++ < retry_times)
	{
		temperature = coul_ntc_temperature_compensation(di);
