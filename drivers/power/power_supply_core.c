...
{
	int ret;
	u32 temp_throttling, temp_shutdown, temp_below_vr_min;
	int i, mask = 0;

	ret = of_property_read_u32(np, "temp_throttling", &temp_throttling);
	if (ret) {
	...
	ret = of_property_read_u32(np, "temp_shutdown", &temp_shutdown);
	if (ret) {
	...
	ret = of_property_read_u32(np, "temp_below_vr_min", &temp_below_vr_min);
	if (ret) {
