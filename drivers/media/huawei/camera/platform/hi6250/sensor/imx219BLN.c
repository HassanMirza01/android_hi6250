...
static hwsensor_vtbl_t s_imx219BLN_vtbl;
...
int imx219BLN_config(hwsensor_intf_t* si, void  *argp);

struct sensor_power_setting imx219BLN_power_up_setting[] = {
...
struct sensor_power_setting imx219BLN_power_down_setting[] = {
    //SCAM RST
    {
        .seq_type = SENSOR_RST,
        .config_val = SENSOR_GPIO_LOW,
...
struct sensor_power_setting imx219BLN_power_up_setting_dvdd120[] = {
    //disable main camera reset
    {
        .seq_type = SENSOR_SUSPEND,
        .config_val = SENSOR_GPIO_LOW,
...
struct sensor_power_setting imx219BLN_power_down_setting_dvdd120[] = {
    //SCAM RST
    {
        .seq_type = SENSOR_RST,
        .config_val = SENSOR_GPIO_LOW,
...
static sensor_t s_imx219BLN =
{
    ...
    .power_setting_array = {
            .size = ARRAY_SIZE(imx219BLN_power_up_setting),
            .power_setting = imx219BLN_power_up_setting,
     },
    .power_down_setting_array = {
            .size = ARRAY_SIZE(imx219BLN_power_down_setting),
            .power_setting = imx219BLN_power_down_setting,
    },
};

static const struct of_device_id s_imx219BLN_dt_match[] =
{
    {
        .compatible = "huawei,imx219BLN",
        .data = &s_imx219BLN.intf,
    },
    {
    },
};

MODULE_DEVICE_TABLE(of, s_imx219BLN_dt_match);

static struct platform_driver s_imx219BLN_driver =
{
    .driver =
    {
        .name = "huawei,imx219BLN",
        .owner = THIS_MODULE,
        .of_match_table = s_imx219BLN_dt_match,
    },
};

char const* imx219BLN_get_name(hwsensor_intf_t* si)
{
    sensor_t* sensor = I2S(si);
    return sensor->board_info->name;
}

int imx219BLN_power_up(hwsensor_intf_t* si)
{
    int ret = 0;
    ...
    ret = hw_sensor_power_up_config(s_imx219BLN.dev, sensor->board_info);
    if (0 == ret ){
...
int imx219BLN_csi_enable(hwsensor_intf_t* si)
{
    return 0;
}

int imx219BLN_csi_disable(hwsensor_intf_t* si)
{
    return 0;
}

static int imx219BLN_match_id(
...
static hwsensor_vtbl_t s_imx219BLN_vtbl =
{
    .get_name = imx219BLN_get_name,
    .config = imx219BLN_config,
    .power_up = imx219BLN_power_up,
    .power_down = imx219BLN_power_down,
    .match_id = imx219BLN_match_id,
    .csi_enable = imx219BLN_csi_enable,
    .csi_disable = imx219BLN_csi_disable,
    .match_id = imx219BLN_match_id,
};
...
static int32_t imx219BLN_platform_probe(struct platform_device* pdev)
{
    int rc = 0;
    cam_notice("enter %s",__func__);

    if (pdev->dev.of_node) {
        rc = hw_sensor_get_dt_data(pdev, &s_imx219BLN);
        if (rc < 0) {
		...
		if(DVDD120V == s_imx219BLN.board_info->ldo_diverse){
			cam_info("%s change to relative power setting line %d\n", __func__, __LINE__);
			s_imx219BLN.power_setting_array.size = ARRAY_SIZE(imx219BLN_power_up_setting_dvdd120);
			s_imx219BLN.power_setting_array.power_setting = imx219BLN_power_up_setting_dvdd120;
			s_imx219BLN.power_down_setting_array.size = ARRAY_SIZE(imx219BLN_power_down_setting_dvdd120);
			s_imx219BLN.power_down_setting_array.power_setting = imx219BLN_power_down_setting_dvdd120;
		}
    } else {
        cam_err("%s imx219BLN of_node is NULL.\n", __func__);
        goto imx219BLN_sensor_probe_fail;
    }
...
static int __init imx219BLN_init_module(void)
{
    cam_info("Enter: %s", __func__);
    return platform_driver_probe(&s_imx219BLN_driver,
            imx219BLN_platform_probe);
}

static void __exit imx219BLN_exit_module(void)
{
    hwsensor_unregister(&s_imx219BLN.intf);
    platform_driver_unregister(&s_imx219BLN_driver);
}

module_init(imx219BLN_init_module);
