...
static int dump_dbg_reg(void)
{
    ...
    for(i = 0 ; i <= JPGENC_FORCE_CLK_ON_CFG_REG; ){
        cam_info("%s: jpeg reg 0x%x = 0x%x", __func__, i, REG_GET(base + i));
        i += 4;
    }
    for(i = JPGENC_DBG_0_REG ; i <= JPGENC_DBG_13_REG; ){
        cam_info("%s: dbg reg 0x%x = 0x%x", __func__, i, REG_GET(base + i));
        i += 4;
    }
#endif
    return 0;
...
{
#ifdef DUMP_REGS
    void __iomem *base = s_hjpeg160.hw_ctl.cvdr_viraddr;//ioremap_nocache();
    void __iomem* smmu_base_addr;

    cam_info("%s: CVDR_SRT_VP_WR_CFG_25 = 0x%x",
            __func__, REG_GET(base + CVDR_SRT_VP_WR_CFG_25_OFFSET));
    cam_info("%s: CVDR_SRT_VP_WR_AXI_LINE_25 = 0x%x",
            __func__, REG_GET(base + CVDR_SRT_VP_WR_AXI_LINE_25_OFFSET));
    cam_info("%s: CVDR_SRT_VP_WR_AXI_FS_25 = 0x%x",
            __func__, REG_GET(base + CVDR_SRT_VP_WR_AXI_FS_25_OFFSET));
    cam_info("%s: CVDR_SRT_LIMITER_VP_WR_25 = 0x%x",
            __func__, REG_GET(base + CVDR_SRT_LIMITER_VP_WR_25_OFFSET));
    cam_info("%s: CVDR_SRT_NR_RD_CFG_1 = 0x%x",
            __func__, REG_GET(base + CVDR_SRT_NR_RD_CFG_1_OFFSET));
    cam_info("%s: CVDR_SRT_LIMITER_NR_RD_1 = 0x%x",
            __func__, REG_GET(base + CVDR_SRT_LIMITER_NR_RD_1_OFFSET));
    cam_info("%s: CVDR_SRT_VP_WR_IF_CFG_25 = 0x%x",
            __func__, REG_GET(base + CVDR_SRT_VP_WR_IF_CFG_25_OFFSET));
    ...
    cam_info("%s: SMMU_GLOBAL_BYPASS = 0x%x",
            __func__, REG_GET(smmu_base_addr + SMMU_GLOBAL_BYPASS));
    cam_info("%s: SMMU_BYPASS_VPWR = 0x%x",
            __func__, REG_GET(smmu_base_addr + SMMU_BYPASS_VPWR));
    cam_info("%s: SMMU_BYPASS_NRRD1 = 0x%x",
            __func__, REG_GET(smmu_base_addr + SMMU_BYPASS_NRRD1));
    cam_info("%s: SMMU_BYPASS_NRRD2 = 0x%x",
            __func__, REG_GET(smmu_base_addr + SMMU_BYPASS_NRRD2));
...
#ifndef SMMU
void hjpeg160_dump_file(char *filename, unsigned long addr, u32 size)
{
#if DUMP_FILE
...
static int hjpeg160_encode_process(hjpeg_intf_t *i, void *cfg)
{
    ...
    if (down_timeout(&s_hjpeg160.buff_done, jiffies)) {
    ...
    byte_cnt = REG_GET(s_hjpeg160.viraddr + JPGENC_JPG_BYTE_CNT_REG );
    if(0 == byte_cnt){
...
static int hjpeg160_power_on(hjpeg_intf_t *i)
{
    ...
    if (NULL == phjpeg160){
        cam_err("%s: phjpeg160 is NULL. (%d)",__func__, __LINE__);
        return -EINVAL;
    }
