...
#define DEPTH_ISP_FW_MAX_PATH_LEN	(64)

#define DEPTH_ISP_FW_LOAD_PATH	("/vendor/firmware/")

/* old style: al6405 compatible, should be removed later */
#define DEPTH_ISP_BOOT_CODE		("miniBoot.bin")
#define DEPTH_ISP_BASIC_CODE	("TBM_Latour.bin")

/* for al6610 fw */
#define DEPTH_ISP_BOOT_BIN		("Boot.bin")
#define DEPTH_ISP_MAIN_BIN		("Main.bin")
#define DEPTH_ISP_SCENARIO_TAB	("SCTable.bin")

/* for al6610 Ocram test */
#define DEPTH_ISP_OCRAM_BOOT	("BootOcram.bin")

extern void ald2_notify_ocram_done(bool flag);
...
int disp_load_fw(void)
{
	...
	struct disp_plat_data *plat_data = NULL;
	int ret = 0, timeout;
	char *fw_boot_name = NULL, *fw_main_name = NULL;
	...
	if (DEPTHISP_AL6045 == plat_data->chip_type) {
		fw_boot_name = DEPTH_ISP_BOOT_CODE;
		fw_main_name = DEPTH_ISP_BASIC_CODE;
	} else {
		/* init register & confirm firmware */
		fw_boot_name = DEPTH_ISP_BOOT_BIN;
		fw_main_name = DEPTH_ISP_MAIN_BIN;
	...
	ret = disp_load_boot_code(drv_data, fw_boot_name);
	if (ret)
		goto out;
	disp_msleep(5);
	...
	ret |= disp_load_basic_code(drv_data, fw_main_name);
	if (ret)
		goto out;
...
int disp_load_ocram_boot(void)
{
	...
	disp_init_regs(drv_data, &init_regs_boot[0], ARRAY_SIZE(init_regs_boot));
	fw_boot_name = DEPTH_ISP_OCRAM_BOOT;
	cam_info("%s: d2_ocram_boot_code(%s)",__func__, fw_boot_name);
...
static int disp_recv_block(struct disp_data *devdata, u8 *out, u32 len)
{
	...
	cam_info("%s - recv block addr=%pK len=%d ", __func__, out, len);
...
static int disp_spi_send(struct disp_data *devdata, u32 len)
{
	...
	if ((!devdata) || (len > SPI_TX_BUF_SIZE)) {
		cam_err("%s - invalid arg devdata=%pK,len=%d",__func__, devdata, len);
		return -EINVAL;
	}
...
static int disp_poll_status(struct disp_data *devdata)
{
	...
	if (!devdata) {
		cam_err("%s - invalid arg devdata=%pK", __func__, devdata);
		return -EINVAL;
	}
...
static int disp_load_boot_code(struct disp_data *devdata, char *name)
{
	...
	loff_t pos;
	char path[DEPTH_ISP_FW_MAX_PATH_LEN];

	memset(path, 0, sizeof(path));

	snprintf(path, DEPTH_ISP_FW_MAX_PATH_LEN-1, "%s%s",/* [false alarm]:buffer overflow will not happen */
			DEPTH_ISP_FW_LOAD_PATH, name);
...
static int disp_probe(struct spi_device *spi)
{
	...
	cam_info("%s tx_buf=0x%pK rx_buf=0x%pK ext_buf=0x%pK, irq_buf=0x%pK",
		__func__,
