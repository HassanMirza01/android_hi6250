...
#define OTP_LSC_GRID_ROWS 17
#define OTP_LSC_GRID_COLS 23
#define OTP_LSC_GRID_POINTS (OTP_LSC_GRID_ROWS*OTP_LSC_GRID_COLS)

#define OTP_LSC_PARAM_BITS 14
// #define OTP_LSC_PARAM_SIZE (OTP_LSC_GRID_POINTS*4*OTP_LSC_PARAM_BITS/8)
...
#define LSC_OTP_SIZE            (OTP_LSC_GRID_POINTS*4*OTP_LSC_PARAM_BITS/8)
#define MAX_HDC_OTP_SIZE      600
#define MAX_STATION_INFO_OTP_SIZE      8*6
...
typedef struct exposure_param_struct
{
    ...
    int banding_type;
    unsigned int expo[2];//shutterspeed
    unsigned int gain[2];
    unsigned int averagevalue[2];
    int EI_info;
    unsigned int iso[2];
    int expo_max;
    int expo_min;
...
typedef struct
{
    union {
        struct {
            ...
            char lsc[LSC_OTP_SIZE]; /* HISI/Altek LSC Gain For Grid */
            ...
            unsigned char otp_lsc_cal_flag;   /* 0x00 means sensor use LSC OTP data, 0xFF means platform use LSC OTP data,
                                                 0xF0 means platform use altek lsc calibration algo*/
            char hdc[MAX_HDC_OTP_SIZE];
            char stationInfo[MAX_STATION_INFO_OTP_SIZE];
        };
