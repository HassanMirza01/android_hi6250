...
int rc_to_ep_by_dma(u32 size, u32 rc_id)
{
	...
	rc_virt_addr = dma_alloc_coherent(g_kirin_pcie[rc_id].pp.dev, size, &rc_phys_addr, GFP_KERNEL | GFP_DMA);
	if (!rc_virt_addr) {
...
static int pcie_test_common_init(void)
{
	...
	for (idx = 0; idx < MAX_RC_NUM; idx++) {
		if (g_kirin_pcie[idx].pp.dev == NULL) {
			PCIE_PR_ERR("g_kirin_pcie[idx].pp.dev == NULL");
			return -1;
		}
		g_pcie_info[idx].rc_virt_addr = dma_alloc_coherent(g_kirin_pcie[idx].pp.dev,
			WIFI_RW_MEMORY_SIZE, &(g_pcie_info[idx].rc_phys_addr), GFP_KERNEL);
		...
		memset(g_pcie_info[idx].rc_virt_addr, 0xb4, WIFI_RW_MEMORY_SIZE);
	}
