	...
	struct ts_cmd_node *out_cmd)
{
	...
	if (buf[0] == ILITEK_ESD_CHECK_DATA && buf[1] == ILITEK_ESD_CHECK_DATA
		&& buf[2] == ILITEK_ESD_CHECK_DATA && buf[3] == ILITEK_ESD_CHECK_DATA ) {
		...
		TS_LOG_ERR("%s ESD  DATA ERR 0x%X, 0x%X, 0x%X, 0x%X ilitek_reset \n", __func__, buf[0], buf[1], buf[2], buf[3]);
		info->cur_finger_number = 0;
		temp_fingers_status = 0;
		return ret;
	}
	...
	TS_LOG_DEBUG("ilitek len = 0x%x buf[0] = 0x%x, buf[1] = 0x%x, buf[2] = 0x%x\n", len, buf[0], buf[1], buf[2]);
	...
	if (ilitek_data->ilitek_roi_enabled) {
		if (pre_fingers_status < temp_fingers_status) {
			...
			ret = ilitek_i2c_write_and_read( buf, 1, ILITEK_WRITE_READ_NOT_DELAY, &(ilitek_data->ilitek_roi_data[4]), ROI_DATA_LENGTH);
			if (ret) {
...
static int ilitek_after_resume(void *feature_info)
{
	...
	ret = ilitek_check_int_low(INT_POLL_SUSPEND_RESUME_RETRY);
	if (ret != ILITEK_INT_STATUS_LOW) {
		TS_LOG_ERR("%s, ilitek_check_int_high fail \n", __func__);
	}
