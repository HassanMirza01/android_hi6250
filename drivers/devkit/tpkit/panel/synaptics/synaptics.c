...
extern int synap_fw_s3718_configid(struct synaptics_rmi4_data *rmi4_data,
				 ...
				 size_t buf_size);
#ifdef SYNA_UPP
extern unsigned short synap_get_oem_data_info( void );
...
static int synaptics_get_NVstructure_cur_index(struct ts_oem_info_param *info, u8 type)
{
	int index = 0;
	int latest_index = 0;
	int flash_size = synap_get_oem_data_info();
...
static int synaptics_chip_detect(struct ts_kit_platform_data *data)
{
	...
	synaptics_power_on();
       ...
       if (retval)
       {
           rmi4_data->synaptics_chip_data->ts_platform_data->client->addr = 0x70;
	...
	if (retval < 0) {
		...
		TS_LOG_INFO("find synaptics device\n");
             strncpy(rmi4_data->synaptics_chip_data->chip_name,SYNAPTICS_VENDER_NAME,strlen(SYNAPTICS_VENDER_NAME)+1);
	}
