...
static int _tsens_register_thermal(void)
{
	...
	for (i = 0, j = 0; i < g_tmdev->tsens_num_sensor; i++, j++) {
		...
		if (np) {
			rc = of_property_read_u32(np, "temp_throttling", &temp_throttling);
			if (rc) {
			...
			rc = of_property_read_u32(np, "temp_shutdown", &temp_shutdown);
			if (rc) {
			...
			rc = of_property_read_u32(np, "temp_below_vr_min", &temp_below_vr_min);
			if (rc) {
