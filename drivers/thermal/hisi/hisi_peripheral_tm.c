...
#ifdef CONFIG_HISI_THERMAL_TRIP
	case TSENS_TRIP_OVER_SKIN:
		tm_sensor->temp_over_skin = (u32)temp;
		return 0;
#endif
	default:
		return -EINVAL;
...
static int hisi_peripheral_tm_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_HISI_THERMAL_TRIP
	char node_name[50] = {0};
	u32 temp_throttling = 0, temp_shutdown = 0, temp_below_vr_min = 0, temp_over_skin = 0;
	...
	for (i = DETECT_SYSTEM_H_CHANEL, flag = DETECT_SYSTEM_H_CHANEL; i < (gtm_dev->tsens_num_sensor + DETECT_SYSTEM_H_CHANEL); i++) {
		...
		if (DETECT_SYSTEM_H_CHANEL == (enum hisi_peripheral_temp_chanel)i) {
			...
			rc = of_property_read_u32(node, node_name, &temp_throttling);
			if (rc) {
			...
			rc = of_property_read_u32(node, node_name, &temp_shutdown);
			if (rc) {
			...
			rc = of_property_read_u32(node, node_name, &temp_below_vr_min);
			if (rc) {
			...
			rc = of_property_read_u32(node, node_name, &temp_over_skin);
			if (rc) {
...
int ipa_get_periph_id(const char *name)
{
	...
	for (id = 0; id < sensor_num; id++) {
		...
		if (!strcmp(name, hisi_peripheral_chanel[id])) {
			ret = (int)id;
			pr_info("sensor_id=%d\n", ret);
			return ret;/*break;*/
		}
