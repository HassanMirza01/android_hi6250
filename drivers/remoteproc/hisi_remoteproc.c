...
static unsigned int get_boardid_version(void)
{
    unsigned int boardid_version[4] = {0};
    struct device_node *root = NULL;
    int ret = 0;

    root = of_find_node_by_path("/");/*lint !e838 */
    if(root==NULL)
        return 1;
    if ((ret = of_property_read_u32_array(root, "hisi,boardid", boardid_version,4)) < 0 ) { /*lint !e747 !e838 */
        RPROC_ERR("Failed: board id of_property_read_u32.%d\n", ret);
        return 1;
    }
    if((boardid_version[0]==6)&&(boardid_version[3]==5)){
        RPROC_INFO("Board ID.%x\n", boardid_version[0]);
        RPROC_INFO("Board ID VER.%x\n", boardid_version[3]);
        return 1;
    }
    return 0;
}
