...
static uint32_t       gregvalue = 0;
static uint32_t       sdevice_type = 1;
				 ...
				 const char *buf, size_t size)
{
	...
	switch (simgdown) {
		...
		case 3:
			...
			ret = slimbus_track_activate(sdevice_type, SLIMBUS_TRACK_IMAGE_LOAD, NULL);
		...
		case 4:
			...
			slimbus_track_deactivate(sdevice_type, SLIMBUS_TRACK_IMAGE_LOAD, NULL);
				 ...
				 const char *buf, size_t size)
{
	...
	if (sregwr >= 0x20007000 && sregwr <= 0x20008000) {
		...
		printk(KERN_ERR"[%s:%d] sregwr (%x %x) !\n", __FUNCTION__, __LINE__, sregwr, gregvalue);
	} else {
		printk(KERN_ERR"[%s:%d] param error! sregwr (%x %x) !", __FUNCTION__, __LINE__, sregwr, gregvalue);
	}
				 ...
				 const char *buf, size_t size)
{
	...
	switch (sswitchframer) {
		case 1:
			slimbus_switch_framer((slimbus_device_type_t)sdevice_type, SLIMBUS_FRAMER_SOC);
			break;
		case 2:
			slimbus_switch_framer((slimbus_device_type_t)sdevice_type, SLIMBUS_FRAMER_CODEC);
				 ...
				 const char *buf, size_t size)
{
	...
	bus_config[SLIMBUS_BUS_CONFIG_SWITCH_FRAMER].cg = (slimbus_clock_gear_t)sclockgear;
				 ...
				 const char *buf, size_t size)
{
	...
	{
		case 1:
			ret = slimbus_track_deactivate((slimbus_device_type_t)sdevice_type, (slimbus_track_type_t)stracktype, NULL);
			break;
		case 2:
			ret = slimbus_track_activate((slimbus_device_type_t)sdevice_type, (slimbus_track_type_t)stracktype, NULL);
			break;
		case 3:
			for (i =0; i<10000;i++) {
				ret = slimbus_track_activate((slimbus_device_type_t)sdevice_type, (slimbus_track_type_t)stracktype, NULL);
				 ...
				 const char *buf, size_t size)
{
	...
	{
		case 0:
			(void)slimbus_pause_clock((slimbus_device_type_t)sdevice_type, SLIMBUS_RT_FAST_RECOVERY);
			break;
		case 1:
			(void)slimbus_pause_clock((slimbus_device_type_t)sdevice_type, SLIMBUS_RT_CONSTANT_PHASE_RECOVERY);
			break;
		case 2:
			(void)slimbus_pause_clock((slimbus_device_type_t)sdevice_type, SLIMBUS_RT_UNSPECIFIED_DELAY);
			break;
		default:
