...
)
{
    ...
    TAF_MEM_SET_S(acCountryListCc, sizeof(acCountryListCc), 0x00, sizeof(acCountryListCc));
    VOS_sprintf_s((VOS_CHAR*)acCountryListCc, sizeof(acCountryListCc), "%d", usCountryListCc);
...
)
{
    ...
    /*号码后移 */
    TAF_MEM_MOVE_S(&(pucAsciiNum[ucInsertDigitsLen]),
                   ((TAF_MO_MAX_ASCII_NUM_LEN + 1 - ucInsertDigitsLen) * sizeof(VOS_UINT8)),
                   &(pucAsciiNum[0]),
                   (ucNumLen * sizeof(VOS_UINT8)));

    /*插入号码 */
    TAF_MEM_CPY_S(pucAsciiNum, (TAF_MO_MAX_ASCII_NUM_LEN + 1), pucInsertDigits, ucInsertDigitsLen);
...
)
{
    ...
    {
        TAF_MEM_MOVE_S(&(pucAsciiNum[0]),
                       (ucNumLen - ucSkipDigitsLen) * sizeof(VOS_UINT8),
                       &(pucAsciiNum[ucSkipDigitsLen]),
                       (ucNumLen - ucSkipDigitsLen) * sizeof(VOS_UINT8));

        /*剩余位清零 */
    TAF_MEM_SET_S(&(pucAsciiNum[ucNumLen - ucSkipDigitsLen]), ucSkipDigitsLen, 0x00, ucSkipDigitsLen);
...
)
{
    ...
    ****************************************************************************************************/
    ...
    TAF_MEM_CPY_S(aucAsciiNum, sizeof(aucAsciiNum), pucAsciiNum, sizeof(aucAsciiNum));
    ...
    /*将转换后的号码信息写回 */
    TAF_MEM_CPY_S(pucAsciiNum, sizeof(aucAsciiNum), aucAsciiNum, sizeof(aucAsciiNum));
    *pucNumLen                = ucNumLen;
...
)
{
    ...
    if (VOS_TRUE == TAF_IsIpcInCountryList(usMccFirstMatchEntryIndex, pucAsciiNum))
    {
        ...
        if (TAF_PLUS_TRANSLATE_DIAL_TYPE_SMS == enDialType)
        {
            ...
            /*号码后移 */
            TAF_MEM_MOVE_S(&(pucAsciiNum[1]),
                           TAF_MT_MAX_ASCII_NUM_LEN * sizeof(VOS_UINT8),
                           &(pucAsciiNum[0]),
                           (ucNumLen * sizeof(VOS_UINT8)));
