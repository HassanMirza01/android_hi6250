...
)
{
    ...
    {
        TAF_MEM_CPY_S(acBs, sizeof(acBs), pstScSiPara->acSib, VOS_StrLen(pstScSiPara->acSib) + 1);
    }
    else if (TAF_CW_SS_CODE == ucNetSsCode)
    {
        TAF_MEM_CPY_S(acBs, sizeof(acBs), pstScSiPara->acSia, VOS_StrLen(pstScSiPara->acSia) + 1);
    }
    else
    {
        acBs[0] = '\0';
