...
VOS_VOID OM_PcvHookInd(VOS_UCHAR* pucBuf, VOS_UINT16 usLen, VOS_UINT16 usBit, VOS_UINT32 ulFrameTick)
{
    ...
    (VOS_VOID)VOS_MemCpy((VOS_UINT8*)pstOmToAppMsg + sizeof(OM_PCV_TRANS_IND_STRU), (VOS_VOID*)pucBuf, usLen);
...
VOS_UINT32 OM_PcvInitRBuf(OM_PCV_RINGBUFFER_STRU *pstRingId, OM_PCV_ADDR_INFO_STRU *pstAddrInfo)
{
    ...
    /* 将ringbuf控制信息写入AHB */
    (VOS_VOID)VOS_MemCpy((VOS_CHAR*)(pstAddrInfo->ulMailBoxAddr),
...
VOS_VOID OM_PcvReleaseAll(VOS_VOID)
{
    ...
    /* ERRORLOG 记录块 */
    (VOS_VOID)VOS_MemSet(&g_stErrLogFlag, 0, sizeof(g_stErrLogFlag));
    return;
}
...
VOS_VOID OM_PcvTransmitTaskEntry( VOS_VOID )
{
    ...
    (VOS_VOID)VOS_MemSet((VOS_CHAR *)(&g_stPcvDebuggingInfo), 0x00, sizeof(OM_PCV_DEBUGGING_INFO_STRU));
