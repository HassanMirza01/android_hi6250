...
VOS_UINT32 CBT_AcpuUsbFrameInit(VOS_VOID)
{
    ...
    (VOS_VOID)VOS_MemSet(&g_stCbtHdlcEncodeBuf, 0, sizeof(g_stCbtHdlcEncodeBuf));
...
{
    g_ulCbtMsgSN       = 0;
    (VOS_VOID)VOS_MemSet(&g_stAcpuCbtCtrlInfo, 0, sizeof(CBT_RCV_CHAN_CTRL_INFO_STRU));
    /* 首包序号为0 */
...
VOS_UINT32 CBT_AcpuSendData(CBT_UNIFORM_MSG_STRU * pstMsg, VOS_UINT16 usMsgLen)
{
    ...
    for (ucCurSegNum = 0; ucCurSegNum < ucMsgCnt-1; ucCurSegNum++)
    {
        ...
        (VOS_VOID)VOS_MemCpy(pucBuf, (VOS_UINT8*)pstMsg, CBT_MSG_HEADER_LENGTH);
        (VOS_VOID)VOS_MemCpy(pucBuf + CBT_MSG_HEADER_LENGTH, (VOS_UINT8*)ulTempAddress, CBT_MSG_SEGMENT_LEN - CBT_MSG_HEADER_LENGTH);
    ...
    (VOS_VOID)VOS_MemCpy(pucBuf, (VOS_UINT8*)pstMsg, CBT_MSG_HEADER_LENGTH);
    (VOS_VOID)VOS_MemCpy(pucBuf + CBT_MSG_HEADER_LENGTH, (VOS_UINT8*)ulTempAddress, usMsgLen);
                            ...
                            VOS_UINT16 usReturnPrimId, VOS_UINT32 ulResult)
{
    ...
    (VOS_VOID)VOS_MemSet(&stCbtToPcMsg, 0, sizeof(CBT_UNIFORM_MSG_STRU));
