...
IPF_AD_DESC_S ADdlshort[IPF_DLAD0_DESC_SIZE];
IPF_AD_DESC_S ADdllong[IPF_DLAD1_DESC_SIZE];
...
void print_CH1_BUFFED_AD_Num(void)
{
	...
	ad0_stat.u32 = ipf_readl(  HI_IPF_CH1_ADQ0_STAT_OFFSET);
	ad1_stat.u32 = ipf_readl(  HI_IPF_CH1_ADQ1_STAT_OFFSET);
...
int IpfDltestIntCb(void)
{
    ...
    if(u32AD0Num > 10)
    {
		u32AD0wptr = ipf_readl(  HI_IPF_CH1_ADQ0_WPTR_OFFSET );
    ...
    if(u32AD1Num > 10)
    {
		u32AD1wptr = ipf_readl(  HI_IPF_CH1_ADQ1_WPTR_OFFSET );
...
int BSP_IPF_ST_config_timeout(void)
{
    ...
    u32OldTime = ipf_readl(HI_IPF_TIME_OUT_OFFSET);
    ...
    if(*(unsigned int*)( HI_IPF_TIME_OUT_OFFSET) != 0x10001)
    {
        IPF_TEST_ERR("bsp_ipf_config_timeout :%d\n",__LINE__);
        goto error;
    }
    ...
    ipf_writel(u32OldTime,   HI_IPF_TIME_OUT_OFFSET);
...
int BSP_IPF_ST_AD_config_test(void)
{
	...
	void *tmp;
	HI_IPF_CH1_ADQ0_WPTR_T adq0_wptr;
	HI_IPF_CH1_ADQ1_WPTR_T adq1_wptr;
	...
	for(i=0; i < IPF_DLAD1_DESC_SIZE; i++)
	{
		...
		if(0 == ADdllong[i].u32OutPtr0){
			IPF_TEST_ERR(" TESTERROR    IPF_ST_UL_INIT ADMEMmallocfail:%d\n",__LINE__);
			goto error2;
		}
	}
	
	adq0_wptr.u32 = ipf_readl(  HI_IPF_CH1_ADQ0_WPTR_OFFSET);
	ad_wptr = adq0_wptr.bits.dl_adq0_wptr;
    s32Ret = mdrv_ipf_config_dlad(IPF_AD_0,IPF_DLAD0_DESC_SIZE-IPF_ADQ_RESERVE_NUM,&ADdlshort[0]);
    if(s32Ret != IPF_SUCCESS){
	...
	adq1_wptr.u32 = ipf_readl(  HI_IPF_CH1_ADQ1_WPTR_OFFSET);
	ad_wptr = adq1_wptr.bits.dl_adq1_wptr;
    s32Ret = mdrv_ipf_config_dlad(IPF_AD_1,IPF_DLAD1_DESC_SIZE-IPF_ADQ_RESERVE_NUM,&ADdllong[0]);
    if(s32Ret != IPF_SUCCESS){
...
int BSP_IPF_ST_100(void)
{
    ...
    if(u32Num <  2) {
        IPF_TEST_PRINT("[Line:%d]Only %d BD!\n",__LINE__, u32Num);
        return ERROR;
    }
...
int BSP_IPF_ST_101(void)
{
    ...
    u32UlBdprt = ipf_readl(  HI_IPF_CH0_BDQ_WPTR_OFFSET);
    IPF_TEST_PRINT("BDQ_WPTR IS %d \n",u32UlBdprt);
	
    u32UlBdprt = ipf_readl(  HI_IPF_CH0_BDQ_RPTR_OFFSET);
...
{	
	...
	ad0_rptr_last = ipf_readl(  HI_IPF_CH0_ADQ0_RPTR_OFFSET);
	ad1_rptr_last = ipf_readl(  HI_IPF_CH0_ADQ1_RPTR_OFFSET);
	while(n>0)
	{
		ret = BSP_IPF_ST_131();
		mdelay(2);
		ad0_rptr_current = ipf_readl(  HI_IPF_CH0_ADQ0_RPTR_OFFSET);
		ad1_rptr_current = ipf_readl(  HI_IPF_CH0_ADQ1_RPTR_OFFSET);
...
void print_ad_status(void)
{
	...
	/*打印下行AD读写指针位置，确定预读状态*/
	current_ad_wptr = ipf_readl(  HI_IPF_CH1_ADQ0_WPTR_OFFSET);
	current_ad_rptr = ipf_readl(  HI_IPF_CH1_ADQ0_RPTR_OFFSET);
	current_ad_state = ipf_readl(  HI_IPF_CH1_ADQ0_STAT_OFFSET);
	...
	current_ad_wptr = ipf_readl(  HI_IPF_CH1_ADQ1_WPTR_OFFSET);
	current_ad_rptr = ipf_readl(  HI_IPF_CH1_ADQ1_RPTR_OFFSET);
	current_ad_state = ipf_readl(  HI_IPF_CH1_ADQ1_STAT_OFFSET);
	if(current_ad_state & IPF_ADQ_BUF_EPT_MASK)
	{
		current_ad_offset = 0; 
	}
...
int BSP_IPF_ST_502(unsigned int target_ad_wptr, unsigned int target_ad_rptr, unsigned int ad_len)
{
	...
	/*reset unreset ipf*/
	ipf_writel(1,   HI_IPF_SRST_OFFSET); 

	ipf_writel(0,   HI_IPF_SRST_OFFSET); 
	...
	/*将AD读写指针配置为特定值*/
	ipf_writel(target_ad_wptr,   HI_IPF_CH1_ADQ0_WPTR_OFFSET);
	ipf_writel(target_ad_rptr,   HI_IPF_CH1_ADQ0_RPTR_OFFSET);
	ipf_writel(target_ad_wptr,   HI_IPF_CH1_ADQ1_WPTR_OFFSET);
	ipf_writel(target_ad_rptr,   HI_IPF_CH1_ADQ1_RPTR_OFFSET);
	
	/*允许下行AD*/
	UPDATE1(reg,	HI_IPF_CH0_ADQ_CTRL,
			ul_adq_en,	IPF_BOTH_ADQ_EN);
