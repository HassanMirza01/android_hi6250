...
u32 nv_writeEx(u32 modem_id, u32 itemid, u32 offset, u8* pdata, u32 datalen)
{
    ...
    if(NV_ID_DRV_IMEI == itemid)
    {
        if((offset == 0) && (datalen == 16))
        {
            char imei[17] = {0};
            (void)nv_imei2str(pdata, imei);
            nv_mntn_record("\n[%s]:modem_id=%d, imei=%s\n",__FUNCTION__,modem_id,imei);
        }
    }
...
u32 bsp_nvm_backup(u32 crc_flag)
{
    ...
    if(NULL == fp)
    {
        ...
        nv_mntn_record("Backup: open %s fail!\n",NV_BACK_PATH);
        goto nv_backup_fail;
    }
    ...
    if(NV_FLAG_NEED_CRC == crc_flag)
    {
        ...
        if(ret)
        {
            ...
            nv_mntn_record("Backup: crc_flag is invalid!\n");
    ...
    if(ret != writeLen)
    {
        ...
        ret = BSP_ERR_NV_WRITE_FILE_FAIL;
        nv_mntn_record("Backup: write fail!write_len=0x%x,ret_len=0x%x.\n",writeLen,ret);
        goto nv_backup_fail;
    }
    ...
    nv_mntn_record("Backup: nvm backup end!\n");
...
u32 bsp_nvm_update_default(void)
{
    ...
    nv_mntn_record("^INFORBU: factory bakup start!\n");

    if(ddr_info->acore_init_state != NV_INIT_OK)
    {
        nv_mntn_record("^INFORBU: factory bakup failed 1, nv is not init! %d\n", ddr_info->acore_init_state);
        return NV_ERROR;
    }


    ret = nv_get_imei_str(NV_DEFAULT_PATH, fac_imei);
    if(ret)
    {
        nv_mntn_record("^INFORBU: factory imei is not exist! 0x%x", ret);
    }
    else
    {
        nv_mntn_record("^INFORBU: factory imei %s\n", fac_imei);
    }

    ret = nv_readEx(1, NV_ID_DRV_IMEI, 0, (u8*)mem_imei, 16);
    if(ret)
    {
        nv_mntn_record("^INFORBU: mem imei is not exist! 0x%x", ret);
    }
    else
    {
        char imei[17];
        (void)nv_imei2str((u8*)mem_imei, imei);
        nv_mntn_record("^INFORBU: mem imei %s\n", imei);
    }
    
    ret = nv_set_resume_mode(NV_MODE_USER);
    if(ret)
    {
        nv_mntn_record("^INFORBU: factory bakup failed 3, set resume mode failed! 0x%x\n", ret);
        return NV_ERROR;
    }
    ...
    if(ret)
    {
        nv_mntn_record("^INFORBU: factory bakup failed 4, ddr check failed! 0x%x\n", ret);
        (void)nv_debug_store_ddr_data();
    ...
    {
        ret = BSP_ERR_NV_NO_FILE;
        nv_mntn_record("^INFORBU: factory bakup failed 5, open factory file failed! 0x%x\n", ret);
        goto nv_update_default_err;
    }
    ...
    if(ret)
    {
        nv_mntn_record("^INFORBU: factory bakup failed 6, ipc sem take failed! 0x%x\n", ret);
        goto nv_update_default_err;
    }
    ...
    {
        ret = BSP_ERR_NV_WRITE_FILE_FAIL;
        nv_mntn_record("^INFORBU: factory bakup failed 7, write factory file failed! 0x%x 0x%x\n", datalen, ddr_info->file_len);
        goto nv_update_default_err;
    }
    ...
    if(ret)
    {
        nv_mntn_record("^INFORBU: factory bakup failed 8, backup failed! 0x%x\n", ret);
        goto nv_update_default_err;
    }
    ...
    if(ret)
    {
        nv_mntn_record("^INFORBU: FETAL ERR! img imei is not equal to factory!\n");
    }
    ...
    if(ret)
    {
        nv_mntn_record("^INFORBU: FETAL ERR! factory imei is not exist! 0x%x", ret);
    }
    else
    {
        nv_mntn_record("^INFORBU: new factory imei %s\n", fac_imei);
    }

    nv_mntn_record("^INFORBU: factory bakup end!\n");
