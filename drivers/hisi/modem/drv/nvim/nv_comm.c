...
u32 nv_write_to_mem(nv_wr_req *wreq, nv_item_info_s *item_info)
{
    ...
    if(nv_crc_need_make_inwr())
    {
        ...
        if(NULL != temp_buff)
        {
            nv_ipc_sem_take(IPC_SEM_NV_CRC, IPC_SME_TIME_OUT);
            /* coverity[secure_coding] */
            memcpy(crc_data, temp_buff, (unsigned long)buff_size);
        ...
        else
        {
            nv_ipc_sem_take(IPC_SEM_NV_CRC, IPC_SME_TIME_OUT);
            /* coverity[secure_coding] */
            memcpy(mdm_base + wreq->offset, wreq->pdata, (unsigned long)wreq->size);
...
u32 nv_get_imei_str(char* path, char* buffer)
{
    ...
    ret = nv_get_key_data(path, NV_ID_DRV_IMEI, imei, (u32)sizeof(imei));
    if(ret)
    {
        return ret;
    }
