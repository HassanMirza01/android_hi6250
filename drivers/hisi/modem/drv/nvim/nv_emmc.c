...
/*lint -save -e785*//*Info 785: (Info -- Too few initializers for aggregate 'g_emmc_info' of type 'struct nv_emmc_global_ctrl_stru')*/
...
u32 nv_sec_file_info_init(const s8* name,struct nv_file_info_stru* sec_info)
{
    u32 ret;
    u32 file_len = 0;
    struct nv_file_info_stru info = {0};
    struct nv_ctrl_file_info_stru ctrl_info = {0};
    u8* file_info;

    /*first: read nv ctrl file*/
    ret = (u32)bsp_blk_read((char*)name, (loff_t)0,&ctrl_info,sizeof(ctrl_info),NULL);
    if(NAND_OK != ret)
    {
    ...
    ret = (u32)bsp_blk_read((char*)name,sizeof(struct nv_ctrl_file_info_stru),file_info, (size_t)ctrl_info.file_size,NULL);
    if(NAND_OK != ret)
    {
    ...
    ret = nv_get_bin_file_len(&ctrl_info,(struct nv_file_list_info_stru*)(unsigned long)file_info,&file_len);
    if(ret)
    {
...
u32 nv_dload_file_info_init(void)
{
    ...
    ret = (u32)bsp_blk_read((char*)NV_DLOAD_SEC_NAME,(loff_t)0,&nv_dload,sizeof(nv_dload),NULL);
    if(ret)
    {
        printf("[%s]:ret 0x%x,\n",__func__,ret);
        return ret;
...
u32 nv_blk_read(struct nv_emmc_file_header_stru* fd,FSZ off,u32 len,u8* ptr)
{
    ...
    ret = (u32)bsp_blk_read((char*)fd->name, offset, ptr, (unsigned long)len,NULL);
...
void show_emmc_info(void)
{
    ...
    printf("\n******************img info*********************\n");
    ret = bsp_blk_read((char*)NV_SYS_SEC_NAME, (loff_t)0,&ctrl_info,sizeof(ctrl_info),NULL);
    if(ret)
        return;
    /* coverity[uninit_use] */
    if(ctrl_info.magicnum != NV_CTRL_FILE_MAGIC_NUM)
    ...
    ret = bsp_blk_read((char*)NV_SYS_SEC_NAME, sizeof(ctrl_info), file_info, (unsigned long)ctrl_info.file_size, NULL);
    if(ret)
    {
    ...
    printf("\n********backup mtd info******\n");
    ret = bsp_blk_read((char*)NV_BACK_SEC_NAME, (loff_t)0,&ctrl_info,sizeof(ctrl_info),NULL);
    if(ret)
    {
        nv_free((void *)file_info);
        return;
    ...
    ret = bsp_blk_read((char*)NV_BACK_SEC_NAME, sizeof(ctrl_info), file_info, (size_t)ctrl_info.file_size,NULL);
    if(ret)
    {
        nv_free((void *)file_info);
        return;
    ...
    printf("\n********default mtd info*****\n");
    ret = bsp_blk_read((char*)NV_DEF_SEC_NAME,(loff_t)0, &ctrl_info, sizeof(ctrl_info),NULL);
    if(ret)
    {
        nv_free((void *)file_info);
        return;
    ...
    ret = bsp_blk_read((char*)NV_DEF_SEC_NAME, sizeof(ctrl_info), file_info, (unsigned long)ctrl_info.file_size,NULL);
    if(ret)
    {
        nv_free((void *)file_info);
        return;
