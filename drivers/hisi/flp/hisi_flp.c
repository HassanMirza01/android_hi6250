        ...
        unsigned int count, unsigned char cmd_type)
{
    ...
    skb = genlmsg_new(count, GFP_ATOMIC);
    if (!skb)
        return -ENOMEM;
    ...
    size_t count, unsigned int type)
{
    ...
    if (!flp_port || (!(flp_port->channel_type & type))) {
        printk(KERN_ERR "flp_port isnot start:%d\n", type);
        return -EINVAL;
    }
    if (FLP_PDR_DATA == type) {
        pdata = &flp_port->pdr_buf;
        if (count%(sizeof(flp_pdr_data_t))) {
            len = (unsigned int)((count)/(sizeof(flp_pdr_data_t))) * (sizeof(flp_pdr_data_t));
        }
    } else {
        pdata = &flp_port->ar_buf;
        if (count%(sizeof(ar_activity_event_t))) {
            len = (unsigned int)((count)/(sizeof(ar_activity_event_t))) * (sizeof(ar_activity_event_t));
        }
    }
    pdata->to_wait = true;
...
static int  flp_ar_stop_cmd(flp_port_t *flp_port, unsigned int timeout, unsigned int  type)
{
    struct list_head    *pos;
    flp_port_t      *port;
    unsigned int count;
...
static int get_pdr_data_from_mcu(const pkt_header_t *head)
{
    ...
    flp_hex_dump((char *)data, (int)len, (char)sizeof(flp_pdr_data_t));
...
#if SAVE_PDR_DATA_TO_FILE
    save_pdr_data_to_file(data, len);
#endif
        ...
        switch (head->cmd)  {
            ...
            case CMD_FLP_RESET_RESP:
                if (FLP_TAG_FLP == flp_port->port_type) {
                    flp_generate_netlink_packet(flp_port, data, sizeof(unsigned int), FLP_GENL_CMD_IOMCU_RESET);
                }
                break;
            default:
...
static void  flp_service_recovery(void)
{
    ...
    list_for_each(pos, &g_flp_dev.list) {
        ...
        if ((FLP_TAG_FLP == flp_port->port_type) && ((flp_port->channel_type & FLP_BATCHING) ||
            ...
            flp_generate_netlink_packet(flp_port, (char *)&response, sizeof(unsigned int), FLP_GENL_CMD_IOMCU_RESET);
        }
    }
    return ;
}
...
static int flp_ar_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd & FLP_IOCTL_CMD_MASK) {
        ...
        case FLP_IOCTL_AR_STOP(0):
            ...
            mutex_lock(&g_flp_dev.lock);
            flp_ar_stop_cmd(flp_port, data, FLP_AR_DATA);
...
static int flp_common_ioctl_open_service(flp_port_t *flp_port)
{
    ...
    if ((g_flp_dev.service_type & FLP_AR_DATA) && (g_flp_dev.ar_start_count)) {
        send_cmd_from_kernel(TAG_AR, CMD_CMN_OPEN_REQ, 0, NULL, (size_t)0);
        send_cmd_from_kernel(TAG_AR, CMD_CMN_CONFIG_REQ, CMD_FLP_AR_START_REQ,
            (char *)&g_flp_dev.ar_mcu_config + sizeof(unsigned int),
            g_flp_dev.ar_mcu_config.event_num * sizeof(ar_activity_config_t) + sizeof(unsigned int));
    }
    ...
    if ((g_flp_dev.service_type & FLP_PDR_DATA) &&
            (g_flp_dev.pdr_start_count)) {
        send_cmd_from_kernel(TAG_PDR, CMD_CMN_OPEN_REQ, 0, NULL, (size_t)0);
        send_cmd_from_kernel(TAG_PDR, CMD_CMN_CONFIG_REQ, CMD_FLP_PDR_START_REQ,
            (char *)&g_flp_dev.pdr_config, sizeof(pdr_start_config_t));
    }
...
static int flp_ioctl_add_geofence(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
    ...
    if (hal_config.length> FLP_GEOFENCE_MAX_NUM) {
    ...
    if (copy_from_user(cmd_data, (void *)hal_config.buf, (unsigned long)hal_config.length)) {
        printk(KERN_ERR "%s copy_from_user error\n", __func__);
        kfree(cmd_data);
        return -EFAULT;
    }
...
static int flp_geofence_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case FLP_IOCTL_GEOFENCE_MODIFY :
            ...
            if (copy_from_user(&modify_config, (void *)arg, sizeof(geofencing_option_info_t))) {
                printk(KERN_ERR "%s copy_from_user error\n", __func__);
                return -EFAULT;
            }
...
static int flp_release(struct inode *inode, struct file *file)
{
    ...
    printk(KERN_ERR "%s pdr_count[%d]:ar_count[%d]:service_type [%d] \n", __func__, g_flp_dev.pdr_start_count,
