...
static enum HISI_PLAT hisi_plat;
...
#define DDR_REG_QOSBUF(m) (REG_BASE_DMSS + 0x8000 + 0x4000 * (m))
#endif
	...
	for (i = 0; i < DDRFLUX_LIST_LEN; i++) {
		...
		case QOSB1:
			...
			if (usr[qosbuf_enable])
				base = dfdev->qosb_base[addr - QOSB0];
		...
		case DMC11:
			...
			if (usr[dmc_enable])
				base = dfdev->dmc_base[addr - DMC00];
...
static int hisi_bw_timer_init(u32 irq_per_cpu)
{
	...
	ret = irq_set_affinity(dfdev->ddrc_flux_timer->irq, cpumask_of(irq_per_cpu));

	if (ret < 0) {
		WARN_ON(1); /*lint !e730*/
		pr_warn("[%s] irq affinity fail,%d\n", __func__, irq_per_cpu);
	}
...
static void check_ddrflux_param(void)
{
	...
	if (usr[ddrc_unsec_pass] == 0) {
		...
		if (WARN_ON(usr[interval] < MIN_STATISTIC_INTERVAL_UNSEC)) {
			pr_info("interval can't too small!\n");
...
static int __ddrflux_reg_iomap(void __iomem	**p, phys_addr_t reg_base, unsigned int size)
{
	if (p && (*p == NULL)) {
		*p = (void __iomem *)VIRT(reg_base, size); /*lint !e647*/
...
static int __ddrflux_reg_iomap_all(void)
{
	...
	if (usr[dmss_enable])
		ret |= __ddrflux_reg_iomap(&dfdev->dmss_base, REG_BASE_DMSS, 8);

	if (usr[qosbuf_enable]) {
		ret |= __ddrflux_reg_iomap(&dfdev->qosb_base[0], DDR_REG_QOSBUF(0), 1);
		ret |= __ddrflux_reg_iomap(&dfdev->qosb_base[1], DDR_REG_QOSBUF(1), 1);
	}

	if (usr[dmc_enable]) {
		ret |= __ddrflux_reg_iomap(&dfdev->dmc_base[0], DDR_REG_DMC(0, 0), 1);
		ret |= __ddrflux_reg_iomap(&dfdev->dmc_base[1], DDR_REG_DMC(0, 1), 1);
		ret |= __ddrflux_reg_iomap(&dfdev->dmc_base[2], DDR_REG_DMC(1, 0), 1);
		ret |= __ddrflux_reg_iomap(&dfdev->dmc_base[3], DDR_REG_DMC(1, 1), 1);
	}
...
{
	__ddrflux_reg_iounmap(&dfdev->dmss_base);

	__ddrflux_reg_iounmap(&dfdev->qosb_base[0]);
	__ddrflux_reg_iounmap(&dfdev->qosb_base[1]);

	__ddrflux_reg_iounmap(&dfdev->dmc_base[0]);
	__ddrflux_reg_iounmap(&dfdev->dmc_base[1]);
	__ddrflux_reg_iounmap(&dfdev->dmc_base[2]);
	__ddrflux_reg_iounmap(&dfdev->dmc_base[3]);
}
...
static int __ddrflux_init_unsec(void)
{
	...
	if ((hisi_plat == HI3660) || (hisi_plat == KIRIN970))
		dfdev->dram_type = (int)LPDDR4;

	pr_info("[%s] mask=0x%x ,dramtype=%d,%d\n", __func__, DDRC_DRAM_TYPE_MASK,
			dfdev->dram_type, __LINE__);
...
void __ddrflux_init(void)
{
	...
	mem_size = (usr[sum_time] / usr[interval] + 1) * (sizeof(struct ddrflux_data));/*lint !e776*/
...
{
	__ddrflux_exit();
	(void)ddr_autofsgt_ctrl(DDR_AUTOFSGT_PROXY_CLIENT_DDRFLUX, DDR_AUTOFSGT_LOGIC_EN);
}
...
{
	(void)ddr_autofsgt_ctrl(DDR_AUTOFSGT_PROXY_CLIENT_DDRFLUX, DDR_AUTOFSGT_LOGIC_DIS);
	__ddrflux_start();
}
...
void ddrflux_view_point_insert(char *slice_name)
{
	if (NULL == slice_name) {
		pr_err("slice error!\n");
		return;
	...
	WARN_ON(SLICE_LEN <= strlen(slice_name));/*[false alarm]*/
	__ddrflux_view_point_insert(slice_name);
}
...
ddrflux_start_store(struct file *filp, const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	if (WARN_ON(!strncmp(init, "0", (unsigned long)0x1))) {
		return (ssize_t)cnt;
	} else if (!strncmp(init, "1", (unsigned long)0x1)) {
		if (flag) {
			hisi_pm_bw_on_timer(usr[interval]);
			pr_info("[%s] sucess!\n", __func__);
		} else {
...
static void dmss_flux_enable_ctrl(int en)
{
	...
	if (!usr[ddrc_unsec_pass] || !usr[dmss_enable])
		return;

	if ((dfdev == NULL) || (dfdev->dmss_base == NULL))
		return;

	if (en) {
		val = readl(SOC_DMSS_GLB_STAT_CTRL_ADDR(dfdev->dmss_base));
		val |= 0xFFF;
		writel(val, SOC_DMSS_GLB_STAT_CTRL_ADDR(dfdev->dmss_base));
	} else {
		val = readl(SOC_DMSS_GLB_STAT_CTRL_ADDR(dfdev->dmss_base));
		val &= ~(0xFFF);
		writel(val, SOC_DMSS_GLB_STAT_CTRL_ADDR(dfdev->dmss_base));
		...
		for (asi_base = 0 ; asi_base <= MAX_DMSS_ASI_BASE ; asi_base++) {
			val = readl(SOC_DMSS_ASI_DYN_CKG_ADDR(dfdev->dmss_base, asi_base));
			ckg_byp = val;
			val |= BIT(SOC_DMSS_ASI_DYN_CKG_ckg_byp_asi_START);
			writel(val, SOC_DMSS_ASI_DYN_CKG_ADDR(dfdev->dmss_base, asi_base));
			writel(ckg_byp, SOC_DMSS_ASI_DYN_CKG_ADDR(dfdev->dmss_base, asi_base));
		}
	}
}
...
static void qosbuf_flux_enable_ctrl(int en)
{
	...
	if (!usr[ddrc_unsec_pass] || !usr[qosbuf_enable])
		return;

	if ((dfdev == NULL) || (dfdev->qosb_base[0] == NULL)
		|| (dfdev->qosb_base[1] == NULL))
		return;

	if (en) {
		val0 = readl(SOC_DDRC_QOSB_QOSB_CFG_PERF_ADDR(dfdev->qosb_base[0]));
		val1 = readl(SOC_DDRC_QOSB_QOSB_CFG_PERF_ADDR(dfdev->qosb_base[1]));
		val0 |= BIT(BIT_QOSB_PERF_EN);
		val1 |= BIT(BIT_QOSB_PERF_EN);
		writel(val0, SOC_DDRC_QOSB_QOSB_CFG_PERF_ADDR(dfdev->qosb_base[0]));
		writel(val1, SOC_DDRC_QOSB_QOSB_CFG_PERF_ADDR(dfdev->qosb_base[1]));
	} else {
		val0 = readl(SOC_DDRC_QOSB_QOSB_CFG_PERF_ADDR(dfdev->qosb_base[0]));
		val1 = readl(SOC_DDRC_QOSB_QOSB_CFG_PERF_ADDR(dfdev->qosb_base[1]));
		val0 &= ~(BIT(BIT_QOSB_PERF_EN));
...
static void dmc_flux_enable_ctrl(int en)
{
	...
	if (!usr[ddrc_unsec_pass] || !usr[dmc_enable])
		return;
	...
	if (en) {
		...
		val00 = readl(SOC_DDRC_DMC_DDRC_CFG_STADAT_ADDR(dfdev->dmc_base[0]));
		val10 = readl(SOC_DDRC_DMC_DDRC_CFG_STADAT_ADDR(dfdev->dmc_base[1]));
		val00 |= BIT(BIT_DMC_EN);
		val10 |= BIT(BIT_DMC_EN);
		writel(val00, SOC_DDRC_DMC_DDRC_CFG_STADAT_ADDR(dfdev->dmc_base[0]));
		writel(val10, SOC_DDRC_DMC_DDRC_CFG_STADAT_ADDR(dfdev->dmc_base[1]));

		/*DMC_0_0&DMC_1_0, offset 0x260, bit30 to 1*/
		val00 = readl(SOC_DDRC_DMC_DDRC_CFG_STACMD_ADDR(dfdev->dmc_base[0]));
		val10 = readl(SOC_DDRC_DMC_DDRC_CFG_STACMD_ADDR(dfdev->dmc_base[1]));
		val00 |= BIT(BIT_DMC_EN);
		val10 |= BIT(BIT_DMC_EN);
		writel(val00, SOC_DDRC_DMC_DDRC_CFG_STACMD_ADDR(dfdev->dmc_base[0]));
		writel(val10, SOC_DDRC_DMC_DDRC_CFG_STACMD_ADDR(dfdev->dmc_base[1]));

		/*DMC_0_1&DMC_1_1, offset 0x254, bit30 to 1*/
		val01 = readl(SOC_DDRC_DMC_DDRC_CFG_STADAT_ADDR(dfdev->dmc_base[2]));
		val11 = readl(SOC_DDRC_DMC_DDRC_CFG_STADAT_ADDR(dfdev->dmc_base[3]));
		val01 |= BIT(BIT_DMC_EN);
		val11 |= BIT(BIT_DMC_EN);
		writel(val01, SOC_DDRC_DMC_DDRC_CFG_STADAT_ADDR(dfdev->dmc_base[2]));
		writel(val11, SOC_DDRC_DMC_DDRC_CFG_STADAT_ADDR(dfdev->dmc_base[3]));
		/*DMC_0_1&DMC_1_1, offset 0x260, bit30 to 1*/
		val01 = readl(SOC_DDRC_DMC_DDRC_CFG_STACMD_ADDR(dfdev->dmc_base[2]));
		val11 = readl(SOC_DDRC_DMC_DDRC_CFG_STACMD_ADDR(dfdev->dmc_base[3]));
		val01 |= BIT(BIT_DMC_EN);
		val11 |= BIT(BIT_DMC_EN);
		writel(val01, SOC_DDRC_DMC_DDRC_CFG_STACMD_ADDR(dfdev->dmc_base[2]));
		writel(val11, SOC_DDRC_DMC_DDRC_CFG_STACMD_ADDR(dfdev->dmc_base[3]));
	} else {
...
#define PRIV_AUTH       (S_IRUSR|S_IWUSR|S_IRGRP)
	...
	if (hisi_plat == KIRIN970) {
		usr[ddrc_unsec_pass] = 1;
	} else {
...
static int hisi_get_platform(struct device_node *np)
{
	...
	if (!strncmp(compatible, "hisilicon,kirin970-ddrc-flux", cmplen + 1)) {
		hisi_plat = KIRIN970;
	} else {
		hisi_plat = HI3660;
	}
...
static int bl31_smem_init(struct device_node *np)
{
	int ret;
	u32 data[2] = {0};
	...
	if (ret) {
		pr_err("[%s] %s node doesn't have bl31-share-mem property!\n",
			   __func__, np->name);
		return -ENODEV;
	}

	bl31_smem_base = HISI_SUB_RESERVED_BL31_SHARE_MEM_PHYMEM_BASE + (phys_addr_t)data[0];
...
static int ddrc_flux_remove(struct platform_device *pdev)
{
	...
	if (NULL != ddrc_flux_dir) {
		debugfs_remove_recursive(ddrc_flux_dir);
	}

	if (NULL != dfdev) {
		devm_kfree(&pdev->dev, dfdev);
	}
...
static struct of_device_id ddrc_flux_of_device_ids[] = {
	...
	{.compatible = "hisilicon,kirin970-ddrc-flux",},
	{},
};
...
/*lint -e528 -esym(528,*)*/
module_platform_driver(ddrc_flux_driver);/*lint   !e64*/
/*lint -e528 +esym(528,*)*/
