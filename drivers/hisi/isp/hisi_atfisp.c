...
#define ISP_SECBASE     SEC_ISP_BASE_ADDR
...
{
#define ISP_MEM_BASE        SEC_ISP_BASE_ADDR
...
static int hisi_atf_getdts(struct platform_device *pdev)
{
    ...
    if ((ret = of_property_read_u32(np, "clk_flag", (unsigned int *)(&dev->clk_flag))) < 0 ) {
        pr_err("Failed : clk_flag.0x%x of_property_read_u32.%d\n", dev->clk_flag, ret);
        return -EINVAL;
    }

    pr_info("[%s] clk_flag.0x%x\n", __func__, dev->clk_flag);
