				...
				unsigned int length, char *buffer)
{
	...
	if ((NULL == partion_name) || (NULL == buffer)) {
		sec_print_err("partion_name(%pK) or buffer(%pK) is null", partion_name, buffer);
		return SEC_ERROR;
	}
					...
					u32 vrl_type)
{
	...
	mutex_lock(&load_image_lock);
	/*get image vrl info*/
	ret  = bsp_read_bin((const char *)image_name, (vrl_index * VRL_SIZE), VRL_SIZE, (void *)SECBOOT_BUFFER);
	if (ret < 0)
	{
...
s32 get_imgname_and_headsize(BSP_CORE_TYPE_E ecoretype, char *image_name, u32 *head_size)
{
	...
	{
		case BSP_CCORE:
			strncpy(image_name, PART_MODEM, (unsigned long)PART_NAMELEN);
			image_name[PART_NAMELEN - 1] = '\0';
			if (NULL != head_size) {
			...
			break;
		case BSP_HIFI:
			strncpy(image_name, PART_FW_HIFI, (unsigned long)PART_NAMELEN);
			image_name[PART_NAMELEN - 1] = '\0';
			if (NULL != head_size) {
			...
			break;
		case BSP_BBE:
			strncpy(image_name, PART_MODEM_DSP, (unsigned long)PART_NAMELEN);
			image_name[PART_NAMELEN - 1] = '\0';
			if (NULL != head_size) {
				*head_size = sizeof(struct image_head);
			}
...
int bsp_image_seccheck (BSP_CORE_TYPE_E ecoretype)
{
	...
	ret = load_vrl_to_os(&session, (SECBOOT_IMG_TYPE)ecoretype, image_name, PrimVRL);
	if (SEC_ERROR == ret) {
...
int bsp_reset_loadimage (BSP_CORE_TYPE_E ecoretype)
{
	...
	if (BSP_BBE == ecoretype || BSP_CDSP == ecoretype) {
		ret = reset_soc_image(&session, (SECBOOT_IMG_TYPE)BSP_CCORE);
	} else {
		ret = reset_soc_image(&session,(SECBOOT_IMG_TYPE)ecoretype);
	}
	if (SEC_ERROR == ret) {
...
int bsp_load_and_verify_image(struct load_image_info *img_info)
{
	...
	for (vrl_type = PrimVRL; vrl_type <= BackVRL; vrl_type++) {
		...
		ret = load_vrl_to_os(&session, (SECBOOT_IMG_TYPE)img_info->ecoretype, img_info->partion_name, vrl_type);
		if (SEC_ERROR == ret) {
			sec_print_err("load_vrl_to_os fail!\n");
			goto err_out;
		}

		ret = load_data_to_os(&session, (SECBOOT_IMG_TYPE)img_info->ecoretype, img_info->partion_name,
		...
		/*end of trans all data, start verify, if success, unreset the SOC*/
		ret = verify_soc_image(&session, (SECBOOT_IMG_TYPE)img_info->ecoretype, img_info->image_addr, SECBOOT_CMD_ID_VERIFY_DATA_TYPE);
		if (SEC_ERROR == ret) {
			if (PrimVRL == vrl_type) {
