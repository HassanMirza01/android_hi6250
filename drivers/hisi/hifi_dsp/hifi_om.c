...
static void hifi_om_show_voice_3a_info(struct work_struct *work)
{
	...
	if (!list_empty(&work_info[work_id].ctl.list)) {
		...
		data_len = om_work->data_len;
		memcpy(data, om_work->data, om_work->data_len);/*lint !e747*/
	...
	case HIFI_3A_INFO_MSG:
		...
		if (!dsm_client_ocuppy(dsm_audio_client)) {
			dsm_client_record(dsm_audio_client, "3a error type:%d error:%d\n", hifi_msg_type, voice_3a_om_info.recv_msg);
			dsm_client_notify(dsm_audio_client, DSM_SOC_HIFI_3A_ERROR);
		}
		break;
	default:
		logi("type(%d), not support\n", hifi_msg_type);
		...
		size_t size, loff_t *ppos)
{
	...
	ret = simple_read_from_buffer(buf, size, ppos,
			LOG_PATH_HIFI_LOG, (strlen(LOG_PATH_HIFI_LOG) + 1));/*lint !e838*/
