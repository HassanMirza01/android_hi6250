...
static const struct noc_dbg_cmd_lookup tbl_noc_dbg_cmd[6] = {
/*   str                        cmd                     */
			  ...
			  void __iomem *reg_addr, uint *reg_value)
{
	...
	case 32:
		writel_relaxed(*reg_value, reg_addr);/*lint !e792*/
		break;
	case 16:
		writew_relaxed(*reg_value, reg_addr);/*lint !e792*/
		break;
	case 8:
		writeb_relaxed((u8)*reg_value, reg_addr);/*lint !e792*/
		break;
	default:
	...
	case E_NOC_DBG_CMD_RD_REG:
		...
		for (i = 0; i < cmdtbl_len; i++) {
			if (!strncmp(reg_str[0],
				tbl_noc_dbg_reg[i].p_reg_name, (size_t)128)) {
		...
		if (match_index >= cmdtbl_len) {
			...
			if (NULL == get_probe_node((const char *)reg_str[1])) {
				/*如果没有输入bus名字，输入默认名字 */
				strncpy(reg_str[1], pt_dbg_reg->node_name, (size_t)50);/*lint !e613*/
		...
		reg_addr = (void __iomem *)((unsigned char *)node->base + pt_dbg_reg->offset);/*lint !e613*/
			  ...
			  size_t bytes, loff_t *off)
{
	...
	kn_buf = kzalloc((size_t)NOC_DBG_STRING_SIZE, GFP_KERNEL);/*lint !e838*/
	if (NULL == kn_buf) {
	...
	byte_writen = simple_write_to_buffer(kn_buf,
					(size_t)NOC_DBG_STRING_SIZE, off,
					userbuf, bytes);/*lint !e838*/
	if (byte_writen <= 0) {
...
int noc_dbg_creat_node(void)
{
	...
	_noc_root = debugfs_create_dir("noc", NULL);/*lint !e838*/
