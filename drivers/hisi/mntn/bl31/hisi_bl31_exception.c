...
{
	struct dentry *g_bl31_dir;

	g_bl31_dir = debugfs_create_dir("bl31_mntn", NULL);
	if (g_bl31_dir) {
		if (NULL == debugfs_create_file("bl31_panic_trigger", 0664, g_bl31_dir, NULL, &bl31_panic_debug_fops)) {
...
static int __init hisi_bl31_panic_init(void)
{
	...
	if (check_himntn(HIMNTN_BL31)) {
		pr_err("%s, HIMNTN_BL31 is closed!\n", __func__);
	...
	if (bl31_create_file() != 0) {
		pr_err("create bl31_panic_trigger node faild.\n");
	}
#endif
