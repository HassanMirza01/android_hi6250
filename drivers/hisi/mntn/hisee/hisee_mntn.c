	...
	void *msg)
{
	...
	if (_msg[0] == HISEE_EXCEPTION) {
		pr_info("%s:exception msg : %x\n", __func__, _msg[0]);
		hisee_exception_modid = _msg[1] + MODID_HISEE_START;
	...
	} else if (_msg[0] == HISEE_IRQ) {
		pr_info("%s:exception msg : %x\n", __func__, _msg[0]);
		hisee_exception_modid = translate_exc_type(_msg[1]);
...
void hisee_mntn_debug_dump(void)
{
	...
	atfd_hisi_service_hisee_mntn_smc((u64)HISEE_MNTN_ID,
		(u64)HISEE_MNTN_GET_LOG, (u64)0x0, (u64)0x0);
	snprintf(path, (size_t)HISEE_MNTN_PATH_MAXLEN, "%s%s/%s", PATH_ROOT, "hisee", HISEE_LOG_FLIENAME);
