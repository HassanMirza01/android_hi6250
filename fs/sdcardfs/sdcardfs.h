...
extern int sdcardfs_setxattr(struct dentry *dentry, const char *name,
			     const void *value, size_t size, int flags);
extern ssize_t sdcardfs_getxattr(struct dentry *dentry, const char *name,
				 void *value, size_t size);
extern ssize_t sdcardfs_listxattr(struct dentry *dentry, char *list,
				  size_t size);
...
static inline void sdcardfs_copy_inode_attr(struct inode *dest,
...
static inline void fix_derived_permission(struct inode *x)
{
	...
	x->i_uid = xfs_uid_to_kuid(SDCARDFS_I(x)->d_uid);
	x->i_gid = xfs_gid_to_kgid(SDCARDFS_I(x)->d_gid);
	...
	if (sbi->options.m_gid == AID_SDCARD_RW) {
		...
		x->i_gid = xfs_gid_to_kgid(AID_SDCARD_RW);
	} else {
		x->i_gid = xfs_gid_to_kgid(multiuser_get_uid(info->userid,
							sbi->options.m_gid));
	}
