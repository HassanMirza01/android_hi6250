...
void asp_test_dma(void)
{
	...
	dma_addr_t dst_dma_addr;
    	...
    	pr_info("asp_test: set_base_addr g_dmac_base_addr = 0x%pK\n",     	g_dmac_base_addr);
	...
	pr_info("asp_test:des_addr_v:0x%pK,src_dma_addr = 0x%x \n",src_addr_v, src_dma_addr);
	pr_info("asp_test:des_addr_v:0x%pK,dst_dma_addr = 0x%x \n",des_addr_v,dst_dma_addr);
	...
	pr_info("asp_test:%s HI3650_ASP_EDMAC_CX_CONFIG_0_REG:%#x\n", __FUNCTION__, value);
		...
		for (i = 0; i < TX_SIZE;i=i+4) {
			...
			pr_info("asp_test:des_addr_v = 0x%pK, value = 0x%x \n",des_addr_v+i, value);
		}
    	}
