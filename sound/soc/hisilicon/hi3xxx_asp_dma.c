...
static void asp_dma_lli_cfg(struct snd_pcm_substream *substream, unsigned int port_index)
{
	...
	for (lli_index = 0; lli_index < dma_lli_num; lli_index++) {
		pr_debug("[%s:%d] port[%d] node[%d]  lli = 0x%x, count = %d, src_addr = 0x%x, des_addr = 0x%x, config = 0x%x\n",
				...
				struct snd_pcm_hw_params *params)
{
	...
	for (port_index = 0; port_index < ports_cnt; port_index++) {
		prtd->pdma_lli_cfg[port_index] = (struct dma_lli_cfg *)dma_alloc_coherent(prtd->pdata->dev,
	...
	for (port_index = 0; port_index < ports_cnt; port_index++) {
		if (prtd->pdma_lli_cfg[port_index]) {
			dma_free_coherent(prtd->pdata->dev, DMA_LLI_NUM * sizeof(struct dma_lli_cfg),
					(void*)prtd->pdma_lli_cfg[port_index], prtd->lli_dma_addr[port_index]);
...
static int hi3xxx_intr_dmac_check_period_irq(struct hi3xxx_asp_dmac_runtime_data *prtd, unsigned int intervalNormalMs, unsigned int dma_channel)
{
	...
	if (prtd->prePeriodTimeNs == 0) {
		...
		if ((periodIntervalMs < intervalNormalMs / 2) || (periodIntervalMs > intervalNormalMs * 2)) {
			pr_warn("[%s:%d] dma period irq error interval %d ms\n", __FUNCTION__, __LINE__,  periodIntervalMs);
			ret = -EFAULT;
		}
	}
...
static int hi3xxx_asp_dmac_new(struct snd_soc_pcm_runtime *rtd)
{
	...
	ret = snd_pcm_lib_preallocate_pages_for_all(pcm, SNDRV_DMA_TYPE_DEV,
			pcm->card->dev, HI3XXX_MAX_BUFFER_SIZE, HI3XXX_MAX_BUFFER_SIZE);
	if (ret) {
