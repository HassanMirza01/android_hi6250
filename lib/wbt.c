...
/*
 * from wait_on_page_writeback to find bio which contain seized page
 * */
static void find_page_in_bio(struct page *page, struct rq_wb *rwb)
{
    if (!rwb)
        return;

    wait_queue_head_t *w = &rwb->wait;
    wait_queue_t *curr, *q;
    unsigned long flags;
    
    if (list_empty_careful(&w->task_list))
        return;

    spin_lock_irqsave(&w->lock, flags);
    list_for_each_entry_safe(curr, q, &w->task_list, task_list)
    {
        if (curr == NULL)
        {
        ...
        struct bio_wait *aio_wait = list_entry(curr, struct bio_wait, wait);
        if (aio_wait == NULL)
        {
            spin_unlock_irqrestore(&w->lock, flags);
            return;
        }
        struct bio *bio_search = aio_wait->bio;
        if (bio_search == NULL)
        {
            spin_unlock_irqrestore(&w->lock, flags);
            return;
        ...
        unsigned short page_count = bio_search->bi_vcnt;

        while (page_count > 0)
        {
            struct bio_vec *prev = &bio_search->bi_io_vec[page_count - 1];

            if (prev->bv_page == NULL)
            {
                spin_unlock_irqrestore(&w->lock, flags);
                return;
            }
...
void f2fs_detect_seized_page(struct page *page, struct super_block *sb)
{
    if (sb->s_bdev == NULL)
        return;

    struct block_device *bi_bdev = sb->s_bdev;
    struct request_queue *q = bdev_get_queue(bi_bdev);
    find_page_in_bio(page, q->rq_wb);
}
EXPORT_SYMBOL(f2fs_detect_seized_page);

/*
 * Block if we will exceed our limit, or if we are currently waiting for
 * the timer to kick off queuing again.
 */
static void __wbt_wait(struct rq_wb *rwb, struct bio *bio, spinlock_t *lock)
{
	/*lint -save -e446*/
	DEFINE_WAIT(wait);
	/*lint -restore*/
    ...
    struct bio_wait aio_wait;

    init_wait(&aio_wait.wait);
#ifdef CONFIG_QUASI
    aio_wait.flag = 0;
#endif
    aio_wait.bio = bio;
	...
	do {
		prepare_to_wait_exclusive(&rwb->wait, &aio_wait.wait,
	...
	finish_wait(&rwb->wait, &aio_wait.wait);
}
